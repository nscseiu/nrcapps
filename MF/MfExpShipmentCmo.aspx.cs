...
{
    public partial class MfExpShipmentCmo : System.Web.UI.Page
    {
                ...
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    foreach (GridViewRow gridRow in GridView1.Rows)
                    {
                        ...
                        { 
                            ...
                            string makeSQLPro = " select ITEM_ID, ITEM_WEIGHT from MF_EXPORT_WBSLIP_CON where WB_SLIP_NO  = '" + gridRow.Cells[0].Text + "'  ";
                            ...
                            // check inventory FG
                            string makeSQL = " select * from MF_FG_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "' ";
                            cmdl = new OracleCommand(makeSQL);
                            oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                            dt = new DataTable();
                            oradata.Fill(dt);
                            ...
                            { 
                                ...
                                string update_inven_mas = "update  MF_FG_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID ";
                                ...
                                string update_ex_invoice = " update  MF_EXPORT_WBSLIP_CON set IS_INVENTORY_STATUS =:IsConfirm,  IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
                                cmdi = new OracleCommand(update_ex_invoice, conn);

                                OracleParameter[] objPrm = new OracleParameter[5];
                                objPrm[0] = cmdi.Parameters.Add("NoWbSlipNo", gridRow.Cells[0].Text);
        ...
        public void Display()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.SEAL_NO, PEWC.REF_ORDER_NO, PP.PARTY_NAME, PI.ITEM_CODE || ' : ' || PI.ITEM_NAME AS ITEM_NAME, PEWC.ITEM_WEIGHT_WB,  PEWC.ITEM_WEIGHT,  NPL.PACKING_NAME, PEWC.PACKING_WEIGHT, PEWC.IS_ACTIVE ,  PEWC.IS_INVENTORY_STATUS,  PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE,  PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM MF_EXPORT_WBSLIP_CON PEWC LEFT JOIN NRC_PACKING_LIST NPL ON NPL.PACKING_ID = PEWC.PACKING_ID LEFT JOIN PF_PARTY PP ON PP.PARTY_ID = PEWC.PARTY_ID LEFT JOIN MF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE  PEWC.IS_INVENTORY_STATUS = 'Transit' AND PEWC.IS_CONFIRM_CHECK = 'Complete' ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  "; // to_char(PEWC.DISPATCH_DATE, 'mm/yyyy') = '" + MonthYear + "' AND
                }
                else
                {
        ...
        public void DisplayComplete()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.IS_SHIPMENT_COMPLETE_DATE, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.SEAL_NO, PEWC.REF_ORDER_NO, PP.PARTY_NAME, PI.ITEM_CODE || ' : ' || PI.ITEM_NAME AS ITEM_NAME, PEWC.ITEM_WEIGHT_WB,  PEWC.ITEM_WEIGHT,  NPL.PACKING_NAME, PEWC.PACKING_WEIGHT, PEWC.IS_ACTIVE ,  PEWC.IS_INVENTORY_STATUS,  PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE,  PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM MF_EXPORT_WBSLIP_CON PEWC LEFT JOIN PF_EXPORT_PACKING_HISTORY PEPH ON PEPH.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN NRC_PACKING_LIST NPL ON NPL.PACKING_ID = PEPH.PACKING_ID LEFT JOIN PF_PARTY PP ON PP.PARTY_ID = PEWC.PARTY_ID LEFT JOIN MF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE  PEWC.IS_INVENTORY_STATUS = 'Complete'  AND PEWC.IS_CONFIRM_CHECK = 'Complete' ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  ";  // to_char(PEWC.DISPATCH_DATE, 'mm/yyyy') = '" + MonthYear + "' AND
                }
                else
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.IS_SHIPMENT_COMPLETE_DATE, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.SEAL_NO, PEWC.REF_ORDER_NO, PP.PARTY_NAME, PI.ITEM_CODE || ' : ' || PI.ITEM_NAME AS ITEM_NAME, PEWC.ITEM_WEIGHT_WB,  PEWC.ITEM_WEIGHT,  NPL.PACKING_NAME, PEWC.PACKING_WEIGHT, PEWC.IS_ACTIVE ,  PEWC.IS_INVENTORY_STATUS,  PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE,  PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM MF_EXPORT_WBSLIP_CON PEWC LEFT JOIN PF_EXPORT_PACKING_HISTORY PEPH ON PEPH.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN NRC_PACKING_LIST NPL ON NPL.PACKING_ID = PEPH.PACKING_ID LEFT JOIN PF_PARTY PP ON PP.PARTY_ID = PEWC.PARTY_ID LEFT JOIN MF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE PEWC.IS_INVENTORY_STATUS = 'Complete'  AND PEWC.IS_CONFIRM_CHECK = 'Complete' AND (PEWC.CONTAINER_NO like '" + txtSearchGrideView2.Text + "%' OR to_char(PEWC.DISPATCH_DATE, 'dd/mm/yyyy')  like '" + txtSearchGrideView2.Text + "%' or  PEWC.CONTAINER_NO like '" + txtSearchGrideView2.Text + "%')  ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  ";
        ...
        protected void linkSelectClick(object sender, EventArgs e)
        {
            ...
            string makeSQL = "  SELECT WB_SLIP_NO, TO_CHAR(IS_SHIPMENT_COMPLETE_DATE, 'DD/MM/YYYY') AS IS_SHIPMENT_COMPLETE_DATE FROM MF_EXPORT_WBSLIP_CON  WHERE WB_SLIP_NO = '" + USER_DATA_ID + "'";
        ...
        public void BtnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    // update data  
                    string update_ex_invoice = " update  MF_EXPORT_WBSLIP_CON set IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:EntryDate, 'DD/MM/YYYY'), IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
                    cmdi = new OracleCommand(update_ex_invoice, conn);
                    ...
                    objPrm[2] = cmdi.Parameters.Add("NoWbSlipNo", TextWbSlipEx.Text);
