...
{
    public partial class MfProductionIssue : System.Web.UI.Page
    {
            ...
            if (Session["USER_NAME"] != null)
            {
                ...
                if (IS_PAGE_ACTIVE == "Enable")
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        string makeDropDownBatchSQL = " SELECT MPBM.BATCH_ID, MPBM.BATCH_NO FROM MF_PRODUCTION_BATCH_MASTER MPBM LEFT JOIN MF_PRODUCTION_ISSUE_MASTER MPIM ON MPIM.BATCH_NO = MPBM.BATCH_NO WHERE MPBM.INVEN_AVAIL_CHECK_S = 'Complete' AND MPIM.BATCH_NO IS NULL ORDER BY MPBM.BATCH_ID DESC";
                        ...
                        string makeDropDownBatch1SQL = " SELECT MPBM.BATCH_ID, MPBM.BATCH_NO FROM MF_PRODUCTION_BATCH_MASTER MPBM LEFT JOIN MF_PRODUCTION_ISSUE_MASTER MPIM ON MPIM.BATCH_NO = MPBM.BATCH_NO WHERE MPBM.INVEN_AVAIL_CHECK_S = 'Complete' ORDER BY MPBM.BATCH_ID DESC";
                        ...
                        string get_batch_id = " select LAST_NUMBER from all_sequences where sequence_name = 'MF_PROD_ISSUE_MASTER_SEQ'";
                        cmdu = new OracleCommand(get_batch_id, conn);
                        int ProductionID = Convert.ToInt32(cmdu.ExecuteScalar().ToString());
                        TextProductionID.Text = Convert.ToString(ProductionID + 1);
        ...
        private void ItemDisplay(object sender, EventArgs e)
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                string makeSQL = " select  row_number() OVER (ORDER BY MI.ITEM_ID) AS SL_NO, MPII.*, ITEM_CODE || ' : ' || ITEM_NAME AS ITEM_NAME, MPIT.TYPE_NAME from MF_PRODUCTION_ISSUE_ITEM MPII LEFT JOIN MF_ITEM MI ON MI.ITEM_ID = MPII.ITEM_ID LEFT JOIN MF_PRODUCTION_ISSUE_TYPE MPIT ON MPIT.TYPE_ID = MPII.TYPE_ID  WHERE MPII.BATCH_NO = '" + BatchNo + "' ORDER BY MI.ITEM_ID,  MPIT.TYPE_ID ";
                ...
                if (Gridview1.Rows.Count > 0)
                {
                    ...
                    DropDownList dt = (DropDownList)Gridview1.FooterRow.FindControl("DropDownTypeID");
                    BindItem(dd, dt, ItemData());
                }
            }  
        ...
        private void BindItem(DropDownList DropDownItemID, DropDownList DropDownTypeID, DataSet ds)
        { 
            ...
            DataSet dct = new DataSet();
            string makeDropDownTypeSQL = " SELECT TYPE_ID, TYPE_NAME  FROM MF_PRODUCTION_ISSUE_TYPE WHERE IS_ACTIVE = 'Enable' ORDER BY TYPE_ID ASC";
            dct = ExecuteBySqlString(makeDropDownTypeSQL);
            dtTypeID = (DataTable)dct.Tables[0];
            DropDownTypeID.DataSource = dtTypeID;
            DropDownTypeID.DataValueField = "TYPE_ID";
            DropDownTypeID.DataTextField = "TYPE_NAME";
            DropDownTypeID.DataBind();
            ...
            { 
                if (e.CommandName == "Insert")
                {
                    ...
                    if (Page.IsValid)
                    {
                            ...
                            int ProductionID = Convert.ToInt32(TextProductionID.Text);
                    
                            var Row = Gridview1.FooterRow;
                            DropDownList DropDownItemID = (DropDownList)Row.FindControl("DropDownItemID");
                            DropDownList DropDownTypeID = (DropDownList)Row.FindControl("DropDownTypeID");
                            TextBox TextFirstWeight = (TextBox)Row.FindControl("TextFirstWeight");
                            TextBox TextSecondWeight = (TextBox)Row.FindControl("TextSecondWeight");
                            TextBox TextItemWeight = (TextBox)Row.FindControl("TextItemWeight");
                            ...
                            int TypeID = Convert.ToInt32(DropDownTypeID.Text);
                        ...
                        string makeRmInSQL = " select * from MF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemID + "' ";
                        ...
                        if (NetWeight <= FinalStock) { 

                        StockOutWetNew = StockOutWet + NetWeight;
                        ...
                        if (0 < RowCount)
                        {
                            ...
                            cmd.Connection = conn;
                            cmd.CommandText = "select * from MF_PRODUCTION_ISSUE_MASTER where BATCH_NO = '" + BatchNumber + "'";
                            cmd.CommandType = CommandType.Text;
                            ...
                            { 
                                ...
                                string get_id = "select MF_PROD_ISSUE_MASTER_SEQ.nextval from dual";
                                cmdu = new OracleCommand(get_id, conn);
                                int newProdID = Int16.Parse(cmdu.ExecuteScalar().ToString());   

                                string insert_grade = "insert into MF_PRODUCTION_ISSUE_MASTER (PRODUCTION_ID, BATCH_NO, ENTRY_DATE, REMARKS, IS_ACTIVE, CREATE_DATE, C_USER_ID) VALUES (:TextProdID, :TextBatchNo, TO_DATE(:EntryDate, 'DD/MM/YYYY'), :TextRemarks, :TextIsActive, TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), :NoCuserID)";
                                ...
                                objPr[2] = cmdi.Parameters.Add("EntryDate", EntryDateNew);
                                ...
                                string update_batch = "update  MF_PRODUCTION_BATCH_MASTER  set PRODUCTION_ISSUE_S = :TextBatchStatus,  PRODUCTION_ISSUE_S_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , PRODUCTION_ISSUE_S_ID = :NoU_USER_ID where BATCH_NO = :NoBatchNo ";
                            ...
                            string get_item_cat_id = "select MF_PROD_ISSUE_ITEM_SEQ.nextval from dual";
                            cmdu = new OracleCommand(get_item_cat_id, conn);
                            int newIssueItemID = Int16.Parse(cmdu.ExecuteScalar().ToString());

                            string insert_user = "insert into MF_PRODUCTION_ISSUE_ITEM (ISSUE_ITEM_ID, PRODUCTION_ID, BATCH_NO, ITEM_ID, TYPE_ID, FIRST_WT, SECOND_WT, ITEM_WEIGHT, CREATE_DATE, C_USER_ID) VALUES ( :NoIssueItemID, :NoProductionID, :TextBatchNo, :TextItemID, :TextTypeID, :TextFirstWeight, :TextSecondWeight, :TextItemWeight, TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), :NoCuserID)";
                            cmdi = new OracleCommand(insert_user, conn);

                            OracleParameter[] objPrm = new OracleParameter[10];
                            objPrm[0] = cmdi.Parameters.Add("NoIssueItemID", newIssueItemID);
                            objPrm[1] = cmdi.Parameters.Add("NoProductionID", ProductionID);
                            objPrm[2] = cmdi.Parameters.Add("TextBatchNo", BatchNumber);
                            objPrm[3] = cmdi.Parameters.Add("TextItemID", ItemID);
                            objPrm[4] = cmdi.Parameters.Add("TextTypeID", TypeID);
                            objPrm[5] = cmdi.Parameters.Add("TextFirstWeight", TextFirstWeight.Text);
                            objPrm[6] = cmdi.Parameters.Add("TextSecondWeight", TextSecondWeight.Text);
                            objPrm[7] = cmdi.Parameters.Add("TextItemWeight", NetWeight);
                            ...
                            alert_box.Controls.Add(new LiteralControl("Insert New Issue For Production Data Successfully"));
                            alert_box.Attributes.Add("class", "alert alert-success alert-dismissible");
                            Display();
                            DisplayProductionIssue();
                            BatchPendingProduction();
                            ItemDisplay(ProductionID, new EventArgs());

                        }
                        else {
                            alert_box.Visible = true;
                            alert_box.Controls.Add(new LiteralControl("Item Weight is not available in the Raw Material Inventory, ID: " + ItemID));
        ...
        try
        {
            ...
            string BatchID = TextProductionID.Text;
            ...
            string update_user = "update  MF_PRODUCTION_BAT_TARGET_ITEM  set BATCH_NO = :TextProductionID, ITEM_ID = :TextItemID, ITEM_WEIGHT_CWT = :TextItemWeight, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , U_USER_ID = :NoU_USER_ID where TARGET_ITEM_ID = :NoTargetItemID ";
            cmdi = new OracleCommand(update_user, conn);

            OracleParameter[] objPrm = new OracleParameter[6]; 
            objPrm[0] = cmdi.Parameters.Add("TextProductionID", BatchNumber);
            ...
            string update_batch = "update  MF_PRODUCTION_BATCH_MASTER  set BATCH_NO = :TextProductionID, FURNACES_ID = :TextFurnacesID, PARTY_ID = :TextDropDownSupplierID, ENTRY_DATE = TO_DATE(:EntryDate, 'DD/MM/YYYY'), REMARKS = :TextRemarks, IS_ACTIVE = :TextIsActive, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , U_USER_ID = :NoU_USER_ID where BATCH_ID = :NoBatchID ";
            cmdi = new OracleCommand(update_batch, conn);

            OracleParameter[] objPr = new OracleParameter[9];
            objPr[0] = cmdi.Parameters.Add("TextProductionID", BatchNumber);
        ...
        protected void Gridview1_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            DropDownList DropDownTypeID = (DropDownList)Gridview1.Rows[e.NewEditIndex].FindControl("DropDownTypeID"); 
            if (DropDownItemID != null)
            {
                BindItem(DropDownItemID, DropDownTypeID, ItemData());
        ...
        try
        {
          if (IS_DELETE_ACTIVE == "Enable")
            ...
            conn.Open();
            int userID = Convert.ToInt32(Session["USER_ID"]);
            int IssueItemID = Convert.ToInt32(Gridview1.DataKeys[e.RowIndex]["ISSUE_ITEM_ID"]);
            int ProductionID = Convert.ToInt32(Gridview1.DataKeys[e.RowIndex]["PRODUCTION_ID"]);
            int ItemID = Convert.ToInt32(Gridview1.DataKeys[e.RowIndex]["ITEM_ID"]);
            string u_date = System.DateTime.Now.ToString("dd-MM-yyyy h:mm:ss tt");
                    // check production data
                    int ItemIdOld = 0; double ItemWeightOld = 0.00; 
                    string makeSQLPro = " select ITEM_ID, ITEM_WEIGHT from MF_PRODUCTION_ISSUE_ITEM where ISSUE_ITEM_ID  = '" + IssueItemID + "'  ";
                    ...
                    // check inventory RM
                    string makeSQL2 = " select * from MF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemID + "'  ";
                    ...
                    if (ItemWeightOld <= FinalStock)
                    {
                        ...
                        string delete_user_page = " delete from MF_PRODUCTION_ISSUE_ITEM where ISSUE_ITEM_ID  = '" + IssueItemID + "'";
          ...
          if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT MPIM.PRODUCTION_ID, MPIM.BATCH_NO, MPIM.ENTRY_DATE, MPIM.CREATE_DATE, MPIM.UPDATE_DATE, MPIM.IS_ACTIVE, MPF.FURNACES_NAME, MP.PARTY_NAME, MI.ITEM_CODE || ' : ' || MI.ITEM_NAME AS ITEM_NAME, MPBM.INVEN_AVAIL_CHECK_S, MPBM.PRODUCTION_ISSUE_S, MPBM.PRODUCTION_ISSUE_POST_S, QUALITY_APPRO_LV_ONE_S FROM MF_PRODUCTION_ISSUE_MASTER MPIM LEFT JOIN MF_PRODUCTION_BATCH_MASTER MPBM ON MPBM.BATCH_NO = MPIM.BATCH_NO LEFT JOIN MF_PRODUCTION_FURNACES MPF ON MPF.FURNACES_ID = MPBM.FURNACES_ID LEFT JOIN MF_PARTY MP ON MP.PARTY_ID = MPBM.PARTY_ID LEFT JOIN MF_PRODUCTION_GRADE MPG ON MPG.GRADE_ID = MPBM.GRADE_ID LEFT JOIN MF_ITEM MI ON MI.ITEM_ID = MPG.ITEM_ID ORDER BY MPIM.PRODUCTION_ID DESC ";
                }
                else
                {
        ...
        protected void LinkSelectClick(object sender, EventArgs e) 
        {
         //   try {  
             ...
             string makeSQL = " SELECT MPIM.PRODUCTION_ID, MPBM.BATCH_ID, MPBM.BATCH_NO, MPBM.FURNACES_ID, MPBM.PARTY_ID, MPBM.GRADE_ID, TO_CHAR(MPIM.ENTRY_DATE,'dd/mm/yyyy') AS ENTRY_DATE, MPIM.IS_ACTIVE, MPIM.REMARKS FROM MF_PRODUCTION_ISSUE_MASTER MPIM LEFT JOIN MF_PRODUCTION_BATCH_MASTER MPBM ON MPBM.BATCH_NO = MPIM.BATCH_NO WHERE MPIM.PRODUCTION_ID = '" + USER_DATA_ID + "'";
             ...
             for (int i = 0; i < RowCount; i++)
             {
                ...
                DataTable dtBatchID = new DataTable();
                DataSet db = new DataSet();
                string makeDropDownBatchSQL = " SELECT MPBM.BATCH_ID, MPBM.BATCH_NO FROM MF_PRODUCTION_BATCH_MASTER MPBM LEFT JOIN MF_PRODUCTION_ISSUE_MASTER MPIM ON MPIM.BATCH_NO = MPBM.BATCH_NO WHERE MPBM.INVEN_AVAIL_CHECK_S = 'Complete' AND MPIM.BATCH_NO = '" + dt.Rows[i]["BATCH_NO"].ToString() + "' ORDER BY MPBM.BATCH_ID DESC";
                db = ExecuteBySqlString(makeDropDownBatchSQL);
                dtBatchID = (DataTable)db.Tables[0];
                DropDownBatchNo.DataSource = dtBatchID;
                DropDownBatchNo.DataValueField = "BATCH_ID";
            ...
            DropDownFurnacesID.Enabled = false;
            DisplayProductionIssue();
            ItemDisplay(TextProductionID.Text, new EventArgs());
         //   }
         //   catch
         //   {
        //        Response.Redirect("~/ParameterError.aspx");
        ...
        protected void DisplayBatchDetalis(object sender, CommandEventArgs e)
        {
            ...
            string makeSQL = " select  MPII.*, MI.ITEM_CODE || ' : ' || MI.ITEM_NAME AS ITEM_NAME, MPIT.TYPE_NAME from MF_PRODUCTION_ISSUE_ITEM MPII LEFT JOIN MF_ITEM MI ON MI.ITEM_ID = MPII.ITEM_ID LEFT JOIN MF_PRODUCTION_ISSUE_TYPE MPIT ON MPIT.TYPE_ID = MPII.TYPE_ID  WHERE  MPII.BATCH_NO = '" + BatchNumber + "' ORDER BY MI.ITEM_ID,  MPII.TYPE_ID ";
