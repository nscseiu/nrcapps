...
{
    public partial class MsExpMaterialPricing : System.Web.UI.Page
    {
       ...
       //    {
            ...
            { 
                ...
                cmdu.Dispose();
                  ...
                  cmd.Connection = conn;
                  cmd.CommandText = "select WB_SLIP_NO from MS_EXPORT_WBSLIP_CON_ITEM WHERE IS_ACTIVE_PRICING IS NULL AND WB_SLIP_NO = '" + DropDownWpSlipNoEx.Text + "'";
                  cmd.CommandType = CommandType.Text;

                  OracleDataReader dr = cmd.ExecuteReader();
                  if (!dr.HasRows)
                  {
                    string update_ex_price = " update  MS_EXPORT_WBSLIP_CON set  IS_ACTIVE_PRICING =:TextActivePricing WHERE WB_SLIP_NO = '" + DropDownWpSlipNoEx.Text + "' ";
        ...
        {
            string query = " SELECT WEECI.EXP_WBCON_ITEM_ID || '-' || WEECI.ITEM_WEIGHT || '-' || WI.ITEM_ID AS EXP_WBCON_ITEM_ID, WEECI.WB_SLIP_NO,  WI.ITEM_NAME || ' - ' || WI.ITEM_CODE || '- Item Weight: ' || WEECI.ITEM_WEIGHT || ', No of Pack: ' || WEECI.NUMBER_OF_PACK  AS ITEM_NAME FROM MS_EXPORT_WBSLIP_CON_ITEM WEECI LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WEECI.ITEM_ID WHERE WEECI.WB_SLIP_NO = :WpSlipNoExID AND IS_ACTIVE_PRICING IS NULL ";
            string strConnString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
        ...
        {
            string query = " SELECT ITEM_SALES_ID, ITEM_SALES_DESCRIPTION, ITEM_ID FROM  MS_SALES_ITEM WHERE ITEM_ID =:ItemID AND IS_ACTIVE = 'Enable' ";
            string strConnString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            using (OracleConnection conn = new OracleConnection(strConnString))
            {
                using (OracleCommand cmd = new OracleCommand(query))
