...
{
    public partial class MsExpSalesInvoice : System.Web.UI.Page
    {
        ...
        double ItemVatAmt = 0.0, TotalInvoiceWt = 0.0, TotalInvoiceAmt = 0.0, TotalInvoiceSrAmt = 0.0, TotalInvoiceQty = 0.0, MatWeight = 0.0; string EntryDateSlip = "", PartyName = "", FullName =""; 
        ...
        List<string> ItempWtPerlist = new List<string>();
        List<string> ItempRatePerlist = new List<string>();
        List<string> ItemDeslistDis;
        int ItemDeslistDisCount, ItempRatelistPerDisCount, ItempWtlistPerDisCount;
            ...
            if (Session["USER_NAME"] != null)
            {
                ...
                if (IS_PAGE_ACTIVE == "Enable")
                {
                  ...
                  //      DropDownInvPrintFor.Items.Insert(0, new ListItem("Select Print For", "0"));
                        ...
                        string makePartySQL = " SELECT PARTY_ID,  PARTY_ID || ' - ' || PARTY_NAME || ' - ' || PARTY_VAT_NO AS PARTY_NAME  FROM MS_PARTY WHERE IS_ACTIVE = 'Enable'  AND IS_SALES_ACTIVE = 'Enable' ORDER BY PARTY_NAME ASC";
                        ...
                        string makePageSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Party Name -' || MP.PARTY_NAME  AS PARTY_NAME FROM MS_EXPORT_WBSLIP_CON PEWC LEFT JOIN MS_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN MS_PARTY MP ON MP.PARTY_ID = PEWC.PARTY_ID WHERE PEWC.EXPORT_INVOICE_NO IS NULL AND PEWCI.IS_ACTIVE_PRICING = 'Enable' GROUP BY PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO || ' - ' || PEWC.WB_SLIP_NO || ', Party Name -' || PARTY_NAME ORDER BY PEWC.WB_SLIP_NO ASC";
                        ...
                        DropDownPartyAddressID.Enabled = false;
        ...
        //  try
         //      {  
                if (IS_ADD_ACTIVE == "Enable")
                {
                    ...
                    int PartyAddressID = Convert.ToInt32(DropDownPartyAddressID.Text);                      
                    ...
                    string get_id = "select MS_EXPORT_SALES_MASTER_ID_SEQ.nextval from dual";
                    ...
                    foreach (ListItem li in DropDownSlipNoEx.Items)
                    {
                        ...
                        if (li.Selected == true)
                        {
                            ...
                            string update_ex_invoice = " update  MS_EXPORT_WBSLIP_CON set EXPORT_INVOICE_NO =:NoExportInvoiceNo, IS_CONFIRM_CHECK =:IsConfirm,  IS_CONFIRM_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_CONFIRM_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
                    ...
                    string insert_ex_sales = "insert into MS_EXPORT_SALES_MASTER (EXPORT_SALES_ID, EXPORT_INVOICE_NO, PAY_TERMS_ID, SHIPPING_INCO_ID, TRADING_VESSEL_ID, TRADING_VESSEL_CODE, SHIPMENT_DATE, SHIPMENT_FROM_TO_ID, PARTY_ID, PARTY_ADDRESS_ID, CREATE_DATE, INVOICE_DATE, C_USER_ID, IS_ACTIVE) VALUES ( :NoExportSalesID, :TextExportInvoiceNo, :NoPayTermID, :NoShippingIncoID, :NoTradingVesselID, :TextVesselCode, TO_DATE(:ShipmentDate, 'DD/MM/YYYY'), :NoShipmentFromToID, :NoPartyID, :NoPartyAddressID, TO_DATE(:c_date, 'DD-MM-YYYY HH:MI:SS AM'), TO_DATE(:InvoiceDate, 'DD/MM/YYYY'), :NoCuserID, :TextIsActive)";
                    cmdi = new OracleCommand(insert_ex_sales, conn); 
                    OracleParameter[] objPr = new OracleParameter[14];
                    ...
                    objPr[9] = cmdi.Parameters.Add("NoPartyAddressID", PartyAddressID);
                    objPr[10] = cmdi.Parameters.Add("InvoiceDate", EntryDateNew2);
                    objPr[11] = cmdi.Parameters.Add("c_date", c_date);
                    objPr[12] = cmdi.Parameters.Add("NoCuserID", userID);
                    objPr[13] = cmdi.Parameters.Add("TextIsActive", ISActive);
          ...
          try
               {  
                if (IS_ADD_ACTIVE == "Enable")
                {
                    ...
                    string get_id = "select MS_EXPORT_CONTRACT_NO_ID_SEQ.nextval from dual";
                    cmdu = new OracleCommand(get_id, conn);
                    int newID = Int32.Parse(cmdu.ExecuteScalar().ToString()); 
                     
                    string insert_ex_sales = "insert into MS_EXPORT_CONTRACT_NO (EXPORT_CONTRACT_NO_ID, CONTRACT_NO, CONTRACT_NO_SERIAL, CONTRACT_DATE, EXPORT_INVOICE_NO, CREATE_DATE, C_USER_ID) VALUES ( :NoContractNoID,  :TextContractNo, :TextContractNoSerial, TO_DATE(:ContractDate, 'DD/MM/YYYY'), :TextExportInvoiceNo, TO_DATE(:c_date, 'DD-MM-YYYY HH:MI:SS AM'),  :NoCuserID)";
                    cmdi = new OracleCommand(insert_ex_sales, conn); 
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string makeContractSQL = " SELECT CONTRACT_NO, CONTRACT_NO_SERIAL, TO_CHAR(CONTRACT_DATE, 'DD-MON-YYYY') AS CONTRACT_DATE FROM MS_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '" + TextExportInvoiceNo.Text + "' "; 
            ...
            string makeContainerSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, NCS.CONTAINER_SIZE_INWORDS FROM MS_EXPORT_WBSLIP_CON PEWC LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID LEFT JOIN MS_EXPORT_SALES_MASTER PESM ON PESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO  WHERE PESM.EXPORT_INVOICE_NO = '" + TextExportInvoiceNo.Text + "' ORDER BY PEWC.CONTAINER_NO ASC "; 
            ...
            string makeContainerItemSQL = " SELECT WSI.ITEM_SALES_DESCRIPTION, SUM((PEWCI.ITEM_WEIGHT / 1000)) AS ITEM_WEIGHT, nvl(PEWCI.MAT_PRICE_PER_MT,0) AS MAT_PRICE_PER_MT, SUM(PEWCI.MATERIAL_AMOUNT) MATERIAL_AMOUNT, PEWCI.CURRENCY_RATE, SUM((PEWCI.CURRENCY_RATE * PEWCI.MAT_PRICE_PER_MT)) AS MAT_CONVERSION_PRICE_PER_MT, SUM(PEWCI.MATERIAL_CONVERSION_AMOUNT)  AS MATERIAL_CONVERSION_AMOUNT FROM MS_EXPORT_WBSLIP_CON_ITEM PEWCI LEFT JOIN NRC_CURRENCY_RATE NCR ON NCR.CURRENCY_RATE_ID = PEWCI.CURRENCY_RATE_ID LEFT JOIN NRC_CURRENCY NCS ON NCS.CURRENCY_ID = NCR.SOURCE_CURRENCY_ID LEFT JOIN NRC_CURRENCY NCT ON NCT.CURRENCY_ID = NCR.TARGET_CURRENCY_ID LEFT JOIN MS_SALES_ITEM WSI ON WSI.ITEM_SALES_ID = PEWCI.ITEM_SALES_ID LEFT JOIN MS_EXPORT_WBSLIP_CON PEWC ON PEWC.WB_SLIP_NO = PEWCI.WB_SLIP_NO LEFT JOIN MS_EXPORT_SALES_MASTER WESM ON WESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO WHERE WESM.EXPORT_INVOICE_NO = '" + TextExportInvoiceNo.Text + "' GROUP BY WSI.ITEM_SALES_DESCRIPTION, PEWCI.MAT_PRICE_PER_MT, PEWCI.CURRENCY_RATE";
            cmdl = new OracleCommand(makeContainerItemSQL);
            oradata = new OracleDataAdapter(cmdl.CommandText, conn);
            di = new DataTable();
            oradata.Fill(di);
            ...
            { 
                ...
                HtmlString += "<div style='float:left;width:792px;height:auto;margin-top:50px;font-family: Tahoma, Arial, Courier, Lucida Sans Typewriter, Lucida Typewriter, monospace; font-size: 12px; font-style: normal; font-variant: normal; font-weight: 400; line-height: 16px;'> ";
              
                HtmlString += "<div style='float:left;width:740px;height:20px; margin:0 5px 0 40px;text-align:center;border-bottom: black solid 1px;' ><span style='font-size:17px;font-weight:700;'>INVOICE</span></div> ";
                HtmlString += "<div style='float:left;width:400px;height:20px; margin:5px 0 0 40px;'><span style='font-size:13px;'> </span></div> ";
                HtmlString += "<div style='float:left;width:335px;height:20px;margin-top:5px;text-align:right;'><span style='font-size:13px;'>VAT NO.: 300507825100003</span></div> ";
  
                HtmlString += "<div style='float:left;width:400px;height:40px; margin:0px 0 0 40px;'><span style='font-size:13px;font-weight: 700;'>INVOICE NO. " + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + " </span></div> ";
                HtmlString += "<div style='float:left;width:335px;height:40px;margin-top:0px;text-align:right;'><span style='font-size:13px;font-weight: 700;'>DATE:  " + dt.Rows[i]["INVOICE_DATE"].ToString() + "</span></div> ";

                HtmlString += "<div style='float:left;width:735px;font-weight:700;margin:0 0 0 40px;'>" + dt.Rows[i]["PARTY_NAME"].ToString() + "</div>";
                HtmlString += "<div style='float:left;width:550px;margin:0 0 0 40px;'>" + dt.Rows[i]["PARTY_ADD_1"].ToString() + "</div>";
                HtmlString += "<div style='float:left;width:735px;margin:0 0 0 40px;'> " + dt.Rows[i]["PARTY_ADD_2"].ToString() + "</BR></BR></div>"; 
            ...
            HtmlString += "<div style='float:left;width:730px;height:auto;margin:0 0 0 40px;-webkit-border-top-left-radius:10px;-webkit-border-top-right-radius:10px;font-size:12px;'> ";

            HtmlString += "<table cellpadding='4px' cellspacing='0' style='font-size: 12px;font-family: Tahoma, Arial, Courier, Lucida Sans Typewriter, Lucida Typewriter, monospace;' >";
            HtmlString += "<th style='border-top:black double 1px;border-left:black double 1px;border-bottom:black solid 1px;border-right:black solid 1px; -webkit-border-top-left-radius:10px;width:90px;'>MARKS & NO. OF CONTAINER (S)</th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;'>SIZE</span></th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;width:270px;'>DESCRIPTION OF MATERIALS</th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;'>NET. WEIGHT IN M/TONS</th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;'><span style='size:12px'>PRICE PER M/TON USD</th> ";
            ...
            HtmlString += "<tr valign='top'> ";
                    ...
                    HtmlString += "</td> "; 
                    HtmlString += "<td style='border-left:black solid 1px;border-right:black solid 1px;text-align:right;'> ";
                    for (int p = 0; p < RowCount3; p++)
                    {
                        HtmlString += "" + string.Format("{0:n2}", Convert.ToDouble(di.Rows[p]["MATERIAL_AMOUNT"].ToString()) ) + "</br> ";
                        TotalInvoiceWt += Convert.ToDouble(di.Rows[p]["ITEM_WEIGHT"].ToString());
               ...
               for (int i=0; i< RowCount; i++) {
                HtmlString += "<tr> ";
                HtmlString += "<td colspan='3' style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;border-left:black solid 1px;text-align:right;font-weight:700;'> TOTAL: </td> "; 
                HtmlString += "<td  style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;text-align:center;font-weight:700;'>" + string.Format("{0:n3}", TotalInvoiceWt) + "</td> ";
                HtmlString += "<td  style='border-top:black solid 1px;border-bottom:black solid 1px;font-weight:700;'></td> ";
                HtmlString += "<td  style='border-top:black solid 1px;border-right:black solid 1px;border-bottom:black solid 1px;border-left:black solid 1px;text-align:right;font-weight:700;'> " + string.Format("{0:n2}", TotalInvoiceAmt) + " </td>";
                HtmlString += "</tr> ";
                string NumberToInWordUSD = NumberToInWords.DecimalToWordsUSD(Convert.ToDecimal(TotalInvoiceAmt)).Trim().ToUpper();
                    HtmlString += "<tr> "; 
                    HtmlString += "<td colspan='6' style='border-left:black solid 1px;text-align:center;border-right:black solid 1px;border-bottom:black solid 1px;-webkit-border-bottom-left-radius:10px;-webkit-border-bottom-right-radius:10px;'>  " + NumberToInWordUSD + " </td> ";
                    HtmlString += "</tr> ";
                HtmlString += "<tr>";
                HtmlString += "<td colspan='5' style='text-align:right;'>SAR: </td> ";
                HtmlString += "<td style='text-align:center;'>" + string.Format("{0:n2}", TotalInvoiceSrAmt) + "</td> ";
                HtmlString += "</tr>";
                HtmlString += "<tr> ";
                HtmlString += "<td colspan='6' style=''></BR>SHIPMENT FROM  " + dt.Rows[i]["SHIPMENT_FROM_NAME"].ToString() + " TO " + dt.Rows[i]["SHIPMENT_TO_NAME"].ToString() + " </BR>	 </td> ";
                HtmlString += "</tr> "; 
                HtmlString += "<tr> ";
                HtmlString += "<td colspan='6' style=''></BR></BR><span style='font-size:15px;font-weight:700;'>NESMA RECYCLING CO. LTD.,</span> </BR> P.O. BOX 54744, JEDDAH 21524, KSA </BR></BR></BR></BR> PREPARED BY</td> ";
                HtmlString += "</tr> ";
            }
                HtmlString += "</table>";
            ...
            string update_user = "update  MS_EXPORT_SALES_MASTER  set IS_PRINT = :TextIsPrint, PRINT_DATE  = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), P_USER_ID = :NoCuserID where EXPORT_SALES_ID = :NoExportSalesID ";
       ...
       //    {
            ...
            { 
                ...
                string makeSlipSQL = " SELECT PEWC.EXPORT_INVOICE_NO FROM MS_EXPORT_SALES_MASTER PESM LEFT JOIN MS_EXPORT_WBSLIP_CON PEWC ON PEWC.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO WHERE  PESM.EXPORT_SALES_ID = '" + ExportSalesID + "'";
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    ...
                    string update_user = "update  MS_EXPORT_WBSLIP_CON set EXPORT_INVOICE_NO =:NoExportInvoiceNo, IS_CONFIRM_CHECK =:IsConfirm,  IS_CONFIRM_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_CONFIRM_ID = :NoCuserID  where EXPORT_INVOICE_NO ='" + ExInvoiceNo + "' ";
                    cmdi = new OracleCommand(update_user, conn);
                ...
                objPr[8] = cmdu.Parameters.Add("NoPartyAddressID", PartyAddressID);
                objPr[9] = cmdu.Parameters.Add("InvoiceDate", EntryDateNew2);
                objPr[10] = cmdu.Parameters.Add("u_date", u_date);
                objPr[11] = cmdu.Parameters.Add("NoCuserID", userID);
                objPr[12] = cmdu.Parameters.Add("TextIsActive", ISActive);
                objPr[13] = cmdu.Parameters.Add("NoExportSalesID", ExportSalesID);
        ...
        protected void DeleteContractClick(object sender, EventArgs e)
        {
            try
            {
                if (IS_DELETE_ACTIVE == "Enable")
                {
                    ...
                    string delete_ex_contract = " delete from MS_EXPORT_CONTRACT_NO where EXPORT_CONTRACT_NO_ID  = '" + USER_DATA_ID + "'"; 
                    cmdi = new OracleCommand(delete_ex_contract, conn); 
                    cmdi.ExecuteNonQuery();
                    cmdi.Parameters.Clear();
                    cmdi.Dispose();
        ...
        { 
            ...
            string makeSQL = " SELECT EXPORT_SALES_ID, EXPORT_INVOICE_NO, PAY_TERMS_ID,  SHIPPING_INCO_ID, TRADING_VESSEL_ID, TRADING_VESSEL_CODE, TO_CHAR(SHIPMENT_DATE,'dd/mm/yyyy') AS SHIPMENT_DATE, TO_CHAR(INVOICE_DATE,'dd/mm/yyyy') AS INVOICE_DATE, PARTY_ID, PARTY_ADDRESS_ID, SHIPMENT_FROM_TO_ID, TO_CHAR(SHIPMENT_DATE,'dd/mm/yyyy') AS SHIPMENT_DATE, CREATE_DATE, UPDATE_DATE, IS_ACTIVE, IS_PRINT, PRINT_DATE FROM MS_EXPORT_SALES_MASTER WHERE EXPORT_SALES_ID = '" + USER_DATA_ID + "'";
             ...
             { 
                ...
                DataTable dtSlipNo = new DataTable();
                DataSet dsp = new DataSet();
                string makePageSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Party Name -' || MP.PARTY_NAME  AS PARTY_NAME FROM MS_EXPORT_WBSLIP_CON PEWC LEFT JOIN MS_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO  LEFT JOIN MS_PARTY MP ON MP.PARTY_ID = PEWC.PARTY_ID  WHERE  PEWCI.IS_ACTIVE_PRICING = 'Enable' AND PEWC.EXPORT_INVOICE_NO = '" + ds.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "'   OR PEWC.EXPORT_INVOICE_NO IS NULL  GROUP BY PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO || ' - ' || PEWC.WB_SLIP_NO || ', Party Name -' || MP.PARTY_NAME ORDER BY PEWC.WB_SLIP_NO ASC"; // AND(PEWC.IS_CONFIRM_CHECK IS NULL AND PEWC.EXPORT_INVOICE_NO IS NULL)  PEWC.EXPORT_INVOICE_NO IS NULL AND
                ...
                DataTable dtPartyAddressID = new DataTable();
                DataSet dsa = new DataSet();
                string makePartyAddIDSQL = " SELECT PARTY_ADDRESS_ID, PARTY_ADD_1 || ', ' || PARTY_ADD_2 AS PARTY_ADDRESS FROM MS_PARTY_ADDRESS WHERE PARTY_ID = '" + ds.Rows[i]["PARTY_ID"].ToString() + "' ORDER BY PARTY_ADDRESS_ID ASC";
                dsa = ExecuteBySqlString(makePartyAddIDSQL);
                dtPartyAddressID = (DataTable)dsa.Tables[0];
                DropDownPartyAddressID.DataSource = dtPartyAddressID;
                DropDownPartyAddressID.DataValueField = "PARTY_ADDRESS_ID";
                DropDownPartyAddressID.DataTextField = "PARTY_ADDRESS";
                DropDownPartyAddressID.DataBind();
                DropDownPartyAddressID.Items.Insert(0, new ListItem("Select Party Address ", "0")); 
                DropDownPartyAddressID.Enabled = true;
                DropDownPartyAddressID.Text = ds.Rows[i]["PARTY_ADDRESS_ID"].ToString(); 
            }
              
             string makeSlipSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Party Name -' || MP.PARTY_NAME AS PARTY_NAME FROM MS_EXPORT_SALES_MASTER PESM LEFT JOIN MS_EXPORT_WBSLIP_CON PEWC ON PEWC.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO LEFT JOIN MS_PARTY MP ON MP.PARTY_ID = PEWC.PARTY_ID WHERE PESM.EXPORT_SALES_ID = '" + USER_DATA_ID + "' ORDER BY PEWC.WB_SLIP_NO ASC  ";
        ...
        public void Display()
        {
            ...
            {
                makeSQL = "SELECT  PESM.EXPORT_SALES_ID, PESM.EXPORT_INVOICE_NO, NPT.PAY_TERMS_NAME, NSI.SHIPPING_INCO_NAME, NTV.TRADING_VESSEL_NAME, PESM.TRADING_VESSEL_CODE, sum(PEWCI.MATERIAL_AMOUNT) AS MATERIAL_AMOUNT, PESM.SHIPMENT_DATE, PESM.INVOICE_DATE,  sum(PEWCI.MATERIAL_CONVERSION_AMOUNT) AS MATERIAL_CONVERSION_AMOUNT, NSLF.SHIPMENT_LOC_NAME AS SHIPMENT_FROM_NAME,  NSLT.SHIPMENT_LOC_NAME AS SHIPMENT_TO_NAME, PESM.CREATE_DATE, PESM.UPDATE_DATE, PESM.IS_ACTIVE, PESM.IS_PRINT, TO_CHAR(PESM.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE, PP.PARTY_NAME, PECN.CONTRACT_NO, sum(PEWCI.ITEM_WEIGHT) AS ITEM_WEIGHT, count(PEWC.WB_SLIP_NO) AS WB_SLIP_NO, count(PEWC.CONTAINER_NO) AS CONTAINER_NO FROM MS_EXPORT_SALES_MASTER PESM LEFT JOIN MS_EXPORT_WBSLIP_CON PEWC ON PEWC.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO LEFT JOIN MS_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN NRC_PAYMENT_TERMS NPT ON NPT.PAY_TERMS_ID = PESM.PAY_TERMS_ID LEFT JOIN MS_PARTY PP ON PP.PARTY_ID = PESM.PARTY_ID LEFT JOIN NRC_SHIPPING_INCOTERMS NSI ON NSI.SHIPPING_INCO_ID = PESM.SHIPPING_INCO_ID LEFT JOIN NRC_TRADING_VESSEL NTV ON NTV.TRADING_VESSEL_ID = PESM.TRADING_VESSEL_ID LEFT JOIN NRC_CURRENCY_RATE NCR ON NCR.CURRENCY_RATE_ID = PEWCI.CURRENCY_RATE_ID LEFT JOIN NRC_CURRENCY NCS ON NCS.CURRENCY_ID = NCR.SOURCE_CURRENCY_ID LEFT JOIN NRC_CURRENCY NCT ON NCT.CURRENCY_ID = NCR.TARGET_CURRENCY_ID LEFT JOIN NRC_SHIPMENT_FROM_TO NSRT ON NSRT.SHIPMENT_FT_ID = PESM.SHIPMENT_FROM_TO_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLF ON NSLF.SHIPMENT_LOC_ID = NSRT.SHIPMENT_FROM_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLT ON NSLT.SHIPMENT_LOC_ID = NSRT.SHIPMENT_TO_ID LEFT JOIN(SELECT EXPORT_INVOICE_NO, COUNT(CONTRACT_NO) AS CONTRACT_NO FROM MS_EXPORT_CONTRACT_NO GROUP BY EXPORT_INVOICE_NO) PECN ON PECN.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO WHERE PEWCI.IS_INVENTORY_STATUS = 'Transit' GROUP BY PESM.EXPORT_SALES_ID, PESM.EXPORT_INVOICE_NO, NPT.PAY_TERMS_NAME, NSI.SHIPPING_INCO_NAME, NTV.TRADING_VESSEL_NAME,  PESM.TRADING_VESSEL_CODE, PESM.SHIPMENT_DATE, PESM.INVOICE_DATE,   NSLF.SHIPMENT_LOC_NAME, NSLT.SHIPMENT_LOC_NAME , PESM.CREATE_DATE, PESM.UPDATE_DATE, PESM.IS_ACTIVE, PESM.IS_PRINT, PESM.PRINT_DATE, PP.PARTY_NAME, PECN.CONTRACT_NO ORDER BY PESM.EXPORT_INVOICE_NO DESC"; 
            }
            else
            {
        ...
        public void DisplayContract()
           {
            ...
            string makeSQL = " SELECT * FROM MS_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '"+ TextExportInvoiceNo.Text  + "' ";
        ...
        public void TextPartyAddress_TextChanged(object sender, EventArgs e)
        {
            OracleConnection conn = new OracleConnection(strConnString);
            conn.Open();
            ...
            DataTable dtPartyAddressID = new DataTable();
            DataSet dsa = new DataSet();
            string makePartyAddIDSQL = " SELECT PARTY_ADDRESS_ID, PARTY_ADD_1 || ', ' || PARTY_ADD_2 AS PARTY_ADDRESS FROM MS_PARTY_ADDRESS WHERE PARTY_ID = '" + DropDownPartyID.Text + "' ORDER BY PARTY_ADDRESS_ID ASC";
            dsa = ExecuteBySqlString(makePartyAddIDSQL);
            dtPartyAddressID = (DataTable)dsa.Tables[0];
            DropDownPartyAddressID.DataSource = dtPartyAddressID;
            DropDownPartyAddressID.DataValueField = "PARTY_ADDRESS_ID";
         ...
         //   DropDownPartyAddressID.Items.Insert(0, new ListItem("Select Party Address ", "0"));
            ...
            string makeSlipSQL = "  SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Party Name -' || MP.PARTY_NAME AS PARTY_NAME FROM  MS_EXPORT_WBSLIP_CON PEWC LEFT JOIN MS_PARTY MP ON MP.PARTY_ID = PEWC.PARTY_ID WHERE PEWC.PARTY_ID  = '" + DropDownPartyID.Text + "' AND PEWC.IS_ACTIVE_PRICING = 'Disable' AND PEWC.EXPORT_INVOICE_NO IS NULL ORDER BY PEWC.WB_SLIP_NO ASC  ";
        ...
        public void TextExportInvoiceNo_TextChanged(object sender, EventArgs e)
        {
            ...
            if (ExportInvoiceNo != null)
            {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandText = "select EXPORT_INVOICE_NO from MS_EXPORT_SALES_MASTER where EXPORT_INVOICE_NO = '" + ExportInvoiceNo + "'";
        ...
        protected void BtnReport_Click(object sender, EventArgs e)
        {
            if (IS_REPORT_ACTIVE == "Enable")
            {
                ...
                string HtmlString = "";
                string makeSQL = " SELECT  PESM.EXPORT_SALES_ID, PESM.EXPORT_INVOICE_NO, NPT.PAY_TERMS_NAME, NSI.SHIPPING_INCO_NAME, NTV.TRADING_VESSEL_NAME, PESM.TRADING_VESSEL_CODE,  TO_CHAR(PESM.SHIPMENT_DATE,'dd-MON-yyyy') AS SHIPMENT_DATE, TO_CHAR(PESM.INVOICE_DATE,'dd-mm-yyyy') AS INVOICE_DATE,  NSLF.SHIPMENT_LOC_NAME AS SHIPMENT_FROM_NAME, NSLT.SHIPMENT_LOC_NAME AS SHIPMENT_TO_NAME, PESM.CREATE_DATE, PESM.UPDATE_DATE, PESM.IS_ACTIVE, PESM.IS_PRINT, PESM.PRINT_DATE, PP.PARTY_NAME, PP.PARTY_ADD_1, PP.PARTY_ADD_2, PP.PARTY_VAT_NO FROM MS_EXPORT_SALES_MASTER PESM LEFT JOIN NRC_PAYMENT_TERMS NPT ON NPT.PAY_TERMS_ID = PESM.PAY_TERMS_ID LEFT JOIN NRC_SHIPPING_INCOTERMS NSI ON NSI.SHIPPING_INCO_ID = PESM.SHIPPING_INCO_ID LEFT JOIN NRC_TRADING_VESSEL NTV ON NTV.TRADING_VESSEL_ID = PESM.TRADING_VESSEL_ID  LEFT JOIN NRC_SHIPMENT_FROM_TO NSRT ON NSRT.SHIPMENT_FT_ID = PESM.SHIPMENT_FROM_TO_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLF ON NSLF.SHIPMENT_LOC_ID = NSRT.SHIPMENT_FROM_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLT ON NSLT.SHIPMENT_LOC_ID = NSRT.SHIPMENT_TO_ID  LEFT JOIN MS_PARTY PP ON PP.PARTY_ID = PESM.PARTY_ID  WHERE TO_CHAR(PESM.SHIPMENT_DATE ,'mm-yyyy') = '" + ShipmentDate + "' ORDER BY PESM.EXPORT_INVOICE_NO ASC ";
                ...
                HtmlString += "<div style='float:left;width:1020px;height:auto;font-size: 14px; font-style: normal; font-variant: normal; font-weight:400; line-height: 16px;'> ";
                HtmlString += "<div style='float:left;width:1020px;height:62px; text-align:center;' ><img src='../../image/logo_from.png' height='98%'/></div> ";
                HtmlString += "<div style='float:left;width:1020px;height:20px; text-align:center;'><span style='font-size:14px;font-weight:700;'>Waste Paper Division</span></div> ";
                HtmlString += "<div style='float:left;width:1020px;height:20px;margin-bottom:15px; text-align:center;border-bottom: black solid 1px;' ><span style='font-size:13px;font-weight:700;'>Shipment of Waste Paper During The Month of " + string.Format("{0: MMMM yyyy}", ShipmentDateNew) + "</span></div> ";
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    string makeContractSQL = " SELECT CONTRACT_NO || '-' || CONTRACT_NO_SERIAL AS CONTRACT_NO, CONTRACT_NO_SERIAL, TO_CHAR(CONTRACT_DATE, 'DD-MON-YYYY') AS CONTRACT_DATE FROM MS_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '" + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "' ";
                    cmdl = new OracleCommand(makeContractSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    dc = new DataTable();
                    oradata.Fill(dc);
                    ...
                    string makeContainerSQL = " SELECT  count(PEWC.CONTAINER_NO) CONTAINER_NO, PI.ITEM_NAME AS ITEM_NAME , SUM((nvl(PEWCI.ITEM_WEIGHT,0) / 1000)) AS ITEM_WEIGHT, nvl(PEWCI.MAT_PRICE_PER_MT,0) AS MAT_PRICE_PER_MT, nvl(SUM(PEWCI.MATERIAL_AMOUNT),0) AS MATERIAL_AMOUNT, SUM((PEWCI.CURRENCY_RATE * PEWCI.MAT_PRICE_PER_MT)) AS MAT_CONVERSION_PRICE_PER_MT, SUM(PEWCI.MATERIAL_CONVERSION_AMOUNT) AS MATERIAL_CONVERSION_AMOUNT FROM MS_EXPORT_SALES_MASTER PESM LEFT JOIN MS_EXPORT_WBSLIP_CON PEWC ON PESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO LEFT JOIN MS_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN MF_ITEM PI ON PI.ITEM_ID = PEWCI.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID LEFT JOIN NRC_CURRENCY_RATE NCR ON NCR.CURRENCY_RATE_ID = PEWCI.CURRENCY_RATE_ID LEFT JOIN NRC_CURRENCY NCS ON NCS.CURRENCY_ID = NCR.SOURCE_CURRENCY_ID LEFT JOIN NRC_CURRENCY NCT ON NCT.CURRENCY_ID = NCR.TARGET_CURRENCY_ID WHERE PESM.EXPORT_INVOICE_NO = '" + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "' GROUP BY PI.ITEM_ID, PI.ITEM_NAME, PEWCI.MAT_PRICE_PER_MT  ORDER BY PI.ITEM_ID ";
                    cmdl = new OracleCommand(makeContainerSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    ds = new DataTable();
                    oradata.Fill(ds);
                    ...
                    for (int x = 0; x < RowCount2; x++)
                    {
                        ...
                        TotalCotainer += Convert.ToInt32(ds.Rows[x]["CONTAINER_NO"].ToString());
                    ...
                    HtmlString += "<tr> ";
                    HtmlString += "<td rowspan=" + RowCount2 + " style='border-left:black solid 1px;border-right:black solid 1px;border-bottom:black solid 1px;text-align:center;'>" + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "</td>";
                    ...
                    for (int l = 0; l < RowCount2; l++)
                    {
                        ...
                        if (l < RowCount2)
                        {
                            HtmlString += "<td style='border-right:black solid 1px;text-align:center;border-bottom:black solid 1px;'>" + ds.Rows[l]["ITEM_NAME"].ToString() + " </td> ";
