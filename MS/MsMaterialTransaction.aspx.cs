...
{
    public partial class MsMaterialTransaction : System.Web.UI.Page
    {
         ...
         //     { 
                ...
                else if(TransactionForID == 3)
                {
                    ...
                    cmdu.Dispose();
                        ...
                        string makeTransSQL = " select * from MS_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
                    ...
                    string update_inven_add = "update  MS_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
          if (IS_PRINT_ACTIVE == "Enable")
            ...
            string HtmlString = "";
            string makeSQL = " SELECT WMT.MAT_TRANS_ID, WMT.TRANSACTION_FOR_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME AS ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, TO_CHAR(WMT.ENTRY_DATE,'dd-MON-yyyy') AS ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  MS_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN MF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE  WMT.MAT_TRANS_ID = '" + TransactionForID + "' ";
            ...
            HtmlString += "<div style='float:left;width:825px;height:20px;text-align:center;font-family:Times New Roman,Times, serif;font-size:17px;font-weight:700;' >Metal Scrap Division</div> ";
            ...
            string update_user = "update  MS_MATERIAL_TRANSACTION  set IS_PRINT = :TextIsPrint, PRINT_DATE  = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), P_USER_ID = :NoCuserID where MAT_TRANS_ID = :NoSlipNoWp ";
        ...
        // try{
            ...
            string makeSQL = " SELECT MAT_TRANS_ID, TRANSACTION_FOR_ID, ITEM_ID, ITEM_WEIGHT, ITEM_TRANSFER_ID, REMARKS, TO_CHAR(ENTRY_DATE,'dd/mm/yyyy') AS ENTRY_DATE, C_USER_ID, CREATE_DATE, U_USER_ID, UPDATE_DATE, IS_ACTIVE, IS_PRINT, P_USER_ID, PRINT_DATE FROM MS_MATERIAL_TRANSACTION WHERE MAT_TRANS_ID = '" + USER_DATA_ID + "'";
            ...
            for (int i = 0; i < RowCount; i++)
            {
                DataTable dtItemID = new DataTable();
                DataSet dsi = new DataSet();
                string makeDropDownItemSQL = " SELECT ITEM_ID, ITEM_CODE || ' : ' || ITEM_NAME AS ITEM_NAME FROM MF_ITEM WHERE ITEM_ID NOT IN  '" + Convert.ToInt32(dt.Rows[i]["ITEM_ID"].ToString()) + "' AND IS_ACTIVE = 'Enable' ORDER BY ITEM_ID ASC";
                dsi = ExecuteBySqlString(makeDropDownItemSQL);
                dtItemID = (DataTable)dsi.Tables[0];
        ...
        public void Display()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = "  SELECT WMT.MAT_TRANS_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME AS ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  MS_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN MF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID ORDER BY  WMT.MAT_TRANS_ID  ";
                }
                else
                {
                    if (DropDownSearchItemID.Text == "0")
                    {
                        makeSQL = " SELECT WMT.MAT_TRANS_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME AS ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  MS_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN MF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID  where WMT.MAT_TRANS_ID like '" + txtSearchEmp.Text + "%' or PI.ITEM_NAME like '" + txtSearchEmp.Text + "%' or to_char(WPM.ENTRY_DATE, 'dd/mm/yyyy') like '" + txtSearchEmp.Text + "%' or to_char(WPM.ENTRY_DATE, 'mm/yyyy')  like '" + txtSearchEmp.Text + "%' or WPM.IS_ACTIVE like '" + txtSearchEmp.Text + "%' ORDER BY WMT.MAT_TRANS_ID asc";  // WPM.CREATE_DATE desc, WPM.UPDATE_DATE desc
                    }
                    else
                    {
                        makeSQL = " SELECT WMT.MAT_TRANS_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME AS ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  MS_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN MF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID where  PI.ITEM_ID like '" + DropDownSearchItemID.Text + "%' AND (WMT.MAT_TRANS_ID like '" + txtSearchEmp.Text + "%' or to_char(WPM.ENTRY_DATE, 'dd/mm/yyyy') like '" + txtSearchEmp.Text + "%' or to_char(WPM.ENTRY_DATE, 'mm/yyyy')  like '" + txtSearchEmp.Text + "%') ORDER BY WMT.MAT_TRANS_ID asc";  // WPM.CREATE_DATE desc, WPM.UPDATE_DATE desc
                    }
        ...
        public void DisplaySummary()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT  NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME AS ITEM_NAME, SUM(WMT.ITEM_WEIGHT) AS ITEM_WEIGHT, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME AS ITEM_NAME_TRANSFER FROM MS_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN MF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN MF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE to_char(WMT.ENTRY_DATE, 'mm/yyyy') = '" + MonthYear + "' GROUP BY NMTF.TRANSACTION_FOR_NAME, WI.ITEM_CODE || ' : ' || WI.ITEM_NAME, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME ORDER BY WI.ITEM_CODE || ' : ' || WI.ITEM_NAME, WIT.ITEM_CODE || ' : ' || WIT.ITEM_NAME   ";
                }
                else
                {
        ...
        //   {
            if (IS_EDIT_ACTIVE == "Enable")
            {
                ...
                // get old purchase item weight
                    ...
                    double InitialStock = 0.00, StockInWet = 0.00, StockOutWet = 0.00, StockOutWetNew = 0.00, FinalStock = 0.00, StockInWetNew = 0.00, FinalStockNew = 0.00;
                    string makeSQL = " select  ITEM_ID, nvl(ITEM_WEIGHT,0) AS ITEM_WEIGHT  from MS_MATERIAL_TRANSACTION WHERE MAT_TRANS_ID  = '" + MatTransactionForID + "'";
                    cmdl = new OracleCommand(makeSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    dt = new DataTable();
                    oradata.Fill(dt);
                ...
                if (TransactionForID == 1)
                {
                    if (ItemWeight <= FinalStock)
                    {
                        ...
                        string update_inven_old = "update  MS_RM_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        ...
                        string makeSQLUpdateRM = " select * from MS_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "'  ";
                ...
                else if (TransactionForID == 2)
                {
                    ...
                    string update_inven_old = "update  MS_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                    cmdl = new OracleCommand(update_inven_old, conn);
                ...
                else if (TransactionForID == 3)
                {
                    if (ItemWeight <= FinalStock)
                    {
                        ...
                        string makeTransCheckSQL = " select * from MS_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
                        ...
                        string update_inven_add_up = "update  MS_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
       ...
       //   {
        ...
        //    {
            if (IS_DELETE_ACTIVE == "Enable")
            {
                ...
                string delete_user = " delete from MS_MATERIAL_TRANSACTION where MAT_TRANS_ID  = '" + MatTransactionForID + "'"; 
