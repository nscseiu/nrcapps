...
namespace NRCAPPS.PF
{
    public partial class PfExpMaterialPricing : System.Web.UI.Page
    {
       ...
       //    {
        ...
        {
            string query = " SELECT ITEM_SALES_ID, ITEM_SALES_DESCRIPTION, ITEM_ID FROM  PF_SALES_ITEM WHERE ITEM_ID =:ItemID AND IS_ACTIVE = 'Enable' ";
            string strConnString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
        ...
        { 
            ...
            DataTable dtSlipNo = new DataTable();
            DataSet dss = new DataSet();
            string makePageSQL = " SELECT PEWC.WB_SLIP_NO || '-' || PEWC.ITEM_WEIGHT || '-' || PEWC.ITEM_ID  AS WB_SLIP_NO,  PEWC.CONTAINER_NO || ' - ' || PEWC.WB_SLIP_NO || ' - ' || PC.PARTY_ID || ' - ' || PC.PARTY_NAME || ' ' || ', Item - ' || PI.ITEM_NAME || ', Sub Item - ' || PSI.SUB_ITEM_NAME || ', Item WT(WB) -' || TO_CHAR(PEWC.ITEM_WEIGHT_WB, '999,999,999') || ', Item WT -' || TO_CHAR((PEWC.ITEM_WEIGHT/1000), '999999.999') AS PARTY_NAME FROM PF_EXPORT_WBSLIP_CON PEWC LEFT JOIN PF_PARTY PC ON PC.PARTY_ID = PEWC.PARTY_ID LEFT JOIN PF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN PF_SUB_ITEM PSI ON PSI.SUB_ITEM_ID = PEWC.SUB_ITEM_ID WHERE PEWC.EXPORT_INVOICE_NO IS NULL AND PEWC.WB_SLIP_NO = '" + USER_DATA_ID[0] + "' ORDER BY PEWC.WB_SLIP_NO ASC";
            dss = ExecuteBySqlString(makePageSQL);
            dtSlipNo = (DataTable)dss.Tables[0];
            DropDownSlipNoEx.DataSource = dtSlipNo;
            DropDownSlipNoEx.DataValueField = "WB_SLIP_NO";
            ...
            string makeSQL = "  SELECT PEWC.WB_SLIP_NO, PEWC.ITEM_ID, PEWC.MAT_PRICE_PER_MT, PEWC.ITEM_SALES_ID, PEWC.MATERIAL_AMOUNT, PEWC.ITEM_WEIGHT, NCR.CURRENCY_RATE_ID || '-' || NCR.EXCHANGE_RATE AS CURRENCY_RATE_ID, PEWC.CURRENCY_RATE, PEWC.MATERIAL_CONVERSION_AMOUNT, PEWC.IS_ACTIVE_PRICING, PEWC.UPDATE_DATE_PRICING, PEWC.U_USER_ID_PRICING FROM PF_EXPORT_WBSLIP_CON PEWC LEFT JOIN  NRC_CURRENCY_RATE NCR ON NCR.CURRENCY_RATE_ID  = PEWC.CURRENCY_RATE_ID WHERE PEWC.WB_SLIP_NO = '" + USER_DATA_ID[0] + "'";
             ...
             for (int i = 0; i < RowCount; i++)
             {
                ...
                string makeSalesItemSQL = " SELECT ITEM_SALES_ID, ITEM_SALES_DESCRIPTION, ITEM_ID FROM PF_SALES_ITEM WHERE ITEM_ID = '" + ds.Rows[i]["ITEM_ID"].ToString() + "' AND IS_ACTIVE = 'Enable'  ";
