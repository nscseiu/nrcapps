...
{
    public partial class PfExpShipmentCmo : System.Web.UI.Page
    {
                ...
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    foreach (GridViewRow gridRow in GridView1.Rows)
                    {
                        ...
                        { 
                            ...
                            string makeSQLPro = " select ITEM_ID, ITEM_WEIGHT from PF_EXPORT_WBSLIP_CON where WB_SLIP_NO  = '" + gridRow.Cells[0].Text + "'  ";
                            ...
                            for (int i = 0; i < RowCount; i++)
                            {
                                ...
                                ItemWeightOld = Convert.ToDouble(dt.Rows[i]["ITEM_WEIGHT"].ToString())/1000;
                            }
                            ...
                            { 
                                ...
                                string update_ex_invoice = " update  PF_EXPORT_WBSLIP_CON set IS_INVENTORY_STATUS =:IsConfirm,  IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
        ...
        public void Display()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    GroupGridView1(GridView1.Rows, 0, 18);
                }
                else
                {
        ...
        void GroupGridView1(GridViewRowCollection gvrc, int startIndex, int total)
        {
            if (total == 0) return;
            int i, count = 1;
            ...
            for (i = 1; i < gvrc.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        ctrl.RowSpan = count;
                        GroupGridView1(new GridViewRowCollection(lst), startIndex + 1, total - 1);
                    }
                    count = 1;
                    lst.Clear();
        ...
        public void DisplayComplete()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.IS_SHIPMENT_COMPLETE_DATE, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.SEAL_NO, PEWC.REL_ORDER_NO, PP.PARTY_NAME,  PI.ITEM_NAME, PSI.SUB_ITEM_NAME, PEWC.ITEM_WEIGHT_WB,  PEWC.ITEM_WEIGHT,  NPL.PACKING_NAME, PEPH.NUMBER_OF_PACK, PEPH.PACK_PER_WEIGHT,  PEPH.TOTAL_WEIGHT, PS.SALESMAN_NAME, PEWC.IS_ACTIVE ,  PEWC.IS_INVENTORY_STATUS,  PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE,  PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM PF_EXPORT_WBSLIP_CON PEWC LEFT JOIN PF_EXPORT_PACKING_HISTORY PEPH ON PEPH.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN NRC_PACKING_LIST NPL ON NPL.PACKING_ID = PEPH.PACKING_ID LEFT JOIN PF_PARTY PP ON PP.PARTY_ID = PEWC.PARTY_ID LEFT JOIN PF_SALESMAN PS ON PS.SALESMAN_ID = PEWC.SALESMAN_ID LEFT JOIN PF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN PF_SUB_ITEM PSI ON PSI.SUB_ITEM_ID = PEWC.SUB_ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE  PEWC.IS_INVENTORY_STATUS = 'Complete'  AND PEWC.IS_CONFIRM_CHECK = 'Complete' ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  ";  // to_char(PEWC.DISPATCH_DATE, 'mm/yyyy') = '" + MonthYear + "' AND
                }
                else
                {
                ...
                if (dt.Rows.Count > 0)
                {
                    GroupGridView2(GridView2.Rows, 0, 18);
                }
                else
                {
        ...
        void GroupGridView2(GridViewRowCollection gvrc, int startIndex, int total)
        {
            if (total == 0) return;
            int i, count = 1;
            ArrayList lst = new ArrayList();
            ...
            for (i = 1; i < gvrc.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        ctrl.RowSpan = count;
                        GroupGridView2(new GridViewRowCollection(lst), startIndex + 1, total - 1);
                    }
                    count = 1;
                    lst.Clear();
                    ctrl = gvrc[i].Cells[startIndex];
        ...
        protected void linkSelectClick(object sender, EventArgs e)
        {
            ...
            string makeSQL = "  SELECT WB_SLIP_NO, TO_CHAR(IS_SHIPMENT_COMPLETE_DATE, 'DD/MM/YYYY') AS IS_SHIPMENT_COMPLETE_DATE FROM PF_EXPORT_WBSLIP_CON  WHERE WB_SLIP_NO = '" + USER_DATA_ID + "'";
        ...
        public void BtnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    // update data  
                    string update_ex_invoice = " update  PF_EXPORT_WBSLIP_CON set IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:EntryDate, 'DD/MM/YYYY'), IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
                    cmdi = new OracleCommand(update_ex_invoice, conn);
