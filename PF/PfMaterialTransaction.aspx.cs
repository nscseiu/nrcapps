...
{
    public partial class PfMaterialTransaction : System.Web.UI.Page
    {
            ...
            if (Session["USER_NAME"] != null)
            {
                ...
                if (IS_PAGE_ACTIVE == "Enable")
                {
                     if (!IsPostBack)
                    {
                        ...
                        string makeDropDownItemSQL = " SELECT ITEM_ID, ITEM_NAME || ' - ' || ITEM_CODE AS ITEM_NAME FROM PF_ITEM WHERE IS_ACTIVE = 'Enable' ORDER BY ITEM_ID ASC";
                        ...
                        string makeDropDownCollecForSQL = " SELECT TRANSACTION_FOR_ID, TRANSACTION_FOR_NAME || ' - ' || SYMBOL AS TRANSACTION_FOR_NAME FROM NRC_MATERIAL_TRANSACTION_FOR WHERE IS_ACTIVE = 'Enable' ORDER BY TRANSACTION_FOR_ID ASC";
        ...
        public void BtnAdd_Click(object sender, EventArgs e)
        {
         ...
         //     { 
                if (IS_ADD_ACTIVE == "Enable")
                {
                    ...
                    if (TransactionForID == 3) { ItemTransferID = Convert.ToInt32(Request.Form[DropDownItemTransferID.UniqueID]); } else { ItemTransferID = 0; }
                ...
                else if (TransactionForID == 3)
                {
                    if (ItemWeight <= FinalStock)
                    {
                        ...
                        StockOutWetNew = StockOutWet + ItemWeight;
                        FinalStockNew = (InitialStock + StockInWet) - StockOutWetNew;
                        string update_inven_trans = "", makeTransSQL = "", update_inven_add = ""; ;
                        if (TransInventoryID == "RM")
                        {
                            update_inven_trans = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";

                        }
                        else
                        {
                            update_inven_trans = "update  PF_FG_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }

                        cmdu = new OracleCommand(update_inven_trans, conn);
                        ...
                        if (TransInventoryID == "RM")
                        {
                            makeTransSQL = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
                        }
                        else
                        {
                            makeTransSQL = " select * from PF_FG_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
                        }
                         
                        cmdl = new OracleCommand(makeTransSQL);
                        oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                        dt = new DataTable();
                        oradata.Fill(dt);
                        RowCount = dt.Rows.Count;
                        ...
                        if (TransInventoryID == "RM")
                        {
                             update_inven_add = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }
                        else
                        {
                             update_inven_add = "update  PF_FG_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }

                        cmdu = new OracleCommand(update_inven_add, conn);

                        OracleParameter[] objPrmInevenAdd = new OracleParameter[5];
                        objPrmInevenAdd[0] = cmdu.Parameters.Add("NoStockIn", StockInWetNew);
                        objPrmInevenAdd[1] = cmdu.Parameters.Add("NoFinalStock", FinalStockNew);
                        objPrmInevenAdd[2] = cmdu.Parameters.Add("u_date", c_date);
                        objPrmInevenAdd[3] = cmdu.Parameters.Add("NoCuserID", userID);
                        objPrmInevenAdd[4] = cmdu.Parameters.Add("NoItemID", ItemTransferID);
                        ...
                        alert_box.Visible = true;
                        alert_box.Controls.Add(new LiteralControl("Insert New Material Transfer Successfully"));
                        alert_box.Attributes.Add("class", "alert alert-success alert-dismissible");
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            if (IS_PRINT_ACTIVE == "Enable")
            {
                ...
                string HtmlString = "";
                string makeSQL = " SELECT WMT.MAT_TRANS_ID, WMT.INVENTORY_TYPE,  WMT.TRANSACTION_FOR_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, TO_CHAR(WMT.ENTRY_DATE,'dd-MON-yyyy') AS ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE  WMT.MAT_TRANS_ID = '" + TransactionForID + "' ";
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    ...
                    HtmlString += "<div style='float:left;width:340px;margin:15px 0 0 20px;text-align:left;' ><span style='font-family:Times New Roman,Times, serif;font-size:15px;font-weight:700;'>Transaction No :</span> <span style='color:Red;font-size:15px;'>" + TransanctionNo + "</span></div> ";
                    HtmlString += "<div style='float:left;width:407px;margin-top:15px;text-align:right;'><span style='font-family:Times New Roman;'>Transaction Date :</span>" + EntryDateSlip + "</div> ";
                    HtmlString += "<div style='float:left;width:750px;margin:6px 0 0 18px;'> ";
                    HtmlString += "<table border=1px; width=100%; cellpadding=5px;>";
                    HtmlString += "<thead>";
                    HtmlString += "<tr>";
                    if (dt.Rows[i]["TRANSACTION_FOR_ID"].ToString() == "3")
                    {
                        HtmlString += "<th><span style='font-family:Times New Roman;'>" + TransanctionFor + " From </span></th>";
                    }
                    else
                    {
                    ...
                    HtmlString += "<th><span style='font-family:Times New Roman;'>Material Weight</span></th>";
                    if (dt.Rows[i]["TRANSACTION_FOR_ID"].ToString() == "3") { HtmlString += "<th><span style='font-family:Times New Roman;'>" + TransanctionFor + " To </span></th>"; }
                    HtmlString += " </tr>";
                    HtmlString += "</thead>";
                    HtmlString += "<tbody>";
                    HtmlString += "<tr>";
                    ...
                    {
                        HtmlString += "<td align='center'>";
                        HtmlString += "" + dt.Rows[i]["ITEM_NAME_TRANSFER"].ToString() + "";
                        HtmlString += "</td>";
                    }
                    HtmlString += "</tr>";
                    HtmlString += "</tbody>";
                ...
                string update_user = "update  PF_MATERIAL_TRANSACTION  set IS_PRINT = :TextIsPrint, PRINT_DATE  = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), P_USER_ID = :NoCuserID where MAT_TRANS_ID = :NoSlipNoWp ";
        ...
        { 
            string query = " SELECT ITEM_ID, ITEM_NAME || ' - ' || ITEM_CODE AS ITEM_NAME FROM PF_ITEM WHERE ITEM_ID NOT IN :ItemID AND IS_ACTIVE = 'Enable' ORDER BY ITEM_ID ASC";
            string strConnString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString; 
        ...
        protected void linkSelectClick(object sender, EventArgs e)
        {
            ...
            string makeSQL = " SELECT MAT_TRANS_ID, TRANSACTION_FOR_ID, ITEM_ID, ITEM_WEIGHT, ITEM_TRANSFER_ID, REMARKS, TO_CHAR(ENTRY_DATE,'dd/mm/yyyy') AS ENTRY_DATE, C_USER_ID, CREATE_DATE, U_USER_ID, UPDATE_DATE, IS_ACTIVE, IS_PRINT, P_USER_ID, PRINT_DATE FROM PF_MATERIAL_TRANSACTION WHERE MAT_TRANS_ID = '" + USER_DATA_ID + "'";
            ...
            for (int i = 0; i < RowCount; i++)
            {
                DataTable dtItemID = new DataTable();
                DataSet dsi = new DataSet();
                string makeDropDownItemSQL = " SELECT ITEM_ID, ITEM_NAME || ' - ' || ITEM_CODE AS ITEM_NAME FROM PF_ITEM WHERE ITEM_ID NOT IN  '" + Convert.ToInt32(dt.Rows[i]["ITEM_ID"].ToString()) + "' AND IS_ACTIVE = 'Enable' ORDER BY ITEM_ID ASC";
                dsi = ExecuteBySqlString(makeDropDownItemSQL);
                dtItemID = (DataTable)dsi.Tables[0];
                DropDownItemTransferID.DataSource = dtItemID;
                DropDownItemTransferID.DataValueField = "ITEM_ID";
                DropDownItemTransferID.DataTextField = "ITEM_NAME";
                DropDownItemTransferID.DataBind();
                DropDownItemTransferID.Items.Insert(0, new ListItem("Select  Item", "0"));
                ...
                if (dt.Rows[i]["TRANSACTION_FOR_ID"].ToString() == "3")
                {
                    TransferItemBox.Style.Remove("display");
                    DropDownItemTransferID.Text = dt.Rows[i]["ITEM_TRANSFER_ID"].ToString();
                }
                else
                {
                    TransferItemBox.Style.Add("display", "none");
                }
            }
            ...
            DropDownItemTransferID.Enabled = false;
        ...
        public void Display()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = "  SELECT WMT.MAT_TRANS_ID, WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID ORDER BY  WMT.MAT_TRANS_ID  ";
                }
                else
                {
                    if (DropDownSearchItemID.Text == "0")
                    {
                        makeSQL = " SELECT WMT.MAT_TRANS_ID, WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID  where WMT.MAT_TRANS_ID like '" + txtSearchEmp.Text + "%' or  WMT.INVENTORY_TYPE like '" + txtSearchEmp.Text + "%' or PP.PARTY_NAME like '" + txtSearchEmp.Text + "%' or PI.ITEM_NAME like '" + txtSearchEmp.Text + "%'  or to_char(WMT.ENTRY_DATE, 'dd/mm/yyyy') like '" + txtSearchEmp.Text + "%' or to_char(WMT.ENTRY_DATE, 'mm/yyyy')  like '" + txtSearchEmp.Text + "%' or WMT.IS_ACTIVE like '" + txtSearchEmp.Text + "%'  ORDER BY WMT.MAT_TRANS_ID asc";  // WMT.CREATE_DATE desc, WMT.UPDATE_DATE desc
                    }
                    else
                    {
                        makeSQL = " SELECT WMT.MAT_TRANS_ID, WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, WMT.ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID where  PI.ITEM_ID like '" + DropDownSearchItemID.Text + "%' AND (WMT.MAT_TRANS_ID like '" + txtSearchEmp.Text + "%' or  WMT.INVENTORY_TYPE like '" + txtSearchEmp.Text + "%'   or to_char(WMT.ENTRY_DATE, 'dd/mm/yyyy') like '" + txtSearchEmp.Text + "%' or to_char(WMT.ENTRY_DATE, 'mm/yyyy')  like '" + txtSearchEmp.Text + "%' or WMT.IS_ACTIVE like '" + txtSearchEmp.Text + "%' ) ORDER BY WMT.MAT_TRANS_ID asc";  // WMT.CREATE_DATE desc, WMT.UPDATE_DATE desc
                    }
        ...
        public void DisplaySummary()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, SUM(WMT.ITEM_WEIGHT) AS ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER FROM PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE to_char(WMT.ENTRY_DATE, 'mm/yyyy') = '" + MonthYear + "' GROUP BY WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WIT.ITEM_NAME ORDER BY WI.ITEM_NAME   ";
                }
                else
                {
                    makeSQL = " SELECT WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, SUM(WMT.ITEM_WEIGHT) AS ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER FROM PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE to_char(WMT.ENTRY_DATE, 'mm/yyyy') = '" + TextMonthYear4.Text + "' GROUP BY WMT.INVENTORY_TYPE, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WIT.ITEM_NAME ORDER BY WI.ITEM_NAME  ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    GridView2.FooterRow.Cells[3].Text = total_wt.ToString("N2");
        ...
        //   {
            if (IS_EDIT_ACTIVE == "Enable")
            {
                ...
                // get old purchase item weight
                    ...
                    double InitialStock = 0.00, StockInWet = 0.00, StockOutWet = 0.00, StockOutWetNew = 0.00, FinalStock = 0.00, StockInWetNew = 0.00, FinalStockNew = 0.00;
                    string makeSQL = " select  ITEM_ID, nvl(ITEM_WEIGHT,0) AS ITEM_WEIGHT  from PF_MATERIAL_TRANSACTION WHERE MAT_TRANS_ID  = '" + MatTransactionForID + "'";
                    cmdl = new OracleCommand(makeSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    dt = new DataTable();
                    oradata.Fill(dt);
                ...
                else if (TransactionForID == 3)
                {
                    if (ItemWeight <= FinalStock)
                    {
                        string update_inven_trans_old = "", makeSQLUpdateTrans = "", update_inven_mas_trans = "", makeTransSQL = "";
                        // add old weight in the inventory
                        StockOutWetNew = StockOutWet - ItemWeightOld;
                        FinalStockNew = (InitialStock + StockInWet) - StockOutWetNew;
                        if (TransInventoryID == "RM")
                        {
                             update_inven_trans_old = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }
                        else
                        {
                             update_inven_trans_old = "update  PF_FG_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }
                        cmdl = new OracleCommand(update_inven_trans_old, conn);
                        ...
                        if (TransInventoryID == "RM")
                        {
                              makeSQLUpdateTrans = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "'  ";
                        }
                        else
                        {
                              makeSQLUpdateTrans = " select * from PF_FG_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "'  ";
                        }
                       
                        cmdl = new OracleCommand(makeSQLUpdateTrans);
                        oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                        dt = new DataTable();
                        oradata.Fill(dt);
                        RowCount = dt.Rows.Count;
                        ...
                        if (TransInventoryID == "RM")
                        {
                             update_inven_mas_trans = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }
                        else
                        {
                             update_inven_mas_trans = "update  PF_FG_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        }
                        cmdu = new OracleCommand(update_inven_mas_trans, conn);
                        ...
                        string update_inven_add = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        cmdu = new OracleCommand(update_inven_add, conn);

                        OracleParameter[] objPrmInevenAdd = new OracleParameter[5];
                        objPrmInevenAdd[0] = cmdu.Parameters.Add("NoStockIn", StockInWetNew);
                        objPrmInevenAdd[1] = cmdu.Parameters.Add("NoFinalStock", FinalStockNew);
                        objPrmInevenAdd[2] = cmdu.Parameters.Add("u_date", u_date);
                        objPrmInevenAdd[3] = cmdu.Parameters.Add("NoCuserID", userID);
                        objPrmInevenAdd[4] = cmdu.Parameters.Add("NoItemID", ItemTransferID);
                        ...
                        string makeTransCheckSQL = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
                        cmdl = new OracleCommand(makeTransCheckSQL);
                        oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                        dt = new DataTable();
                        oradata.Fill(dt);
                        RowCount = dt.Rows.Count;
                        ...
                        string update_inven_add_up = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        cmdu = new OracleCommand(update_inven_add_up, conn);

                        OracleParameter[] objPrmInevenAddUp = new OracleParameter[5];
                        objPrmInevenAddUp[0] = cmdu.Parameters.Add("NoStockIn", StockInWetNew);
                        objPrmInevenAddUp[1] = cmdu.Parameters.Add("NoFinalStock", FinalStockNew);
                        objPrmInevenAddUp[2] = cmdu.Parameters.Add("u_date", u_date);
                        objPrmInevenAddUp[3] = cmdu.Parameters.Add("NoCuserID", userID);
                        objPrmInevenAddUp[4] = cmdu.Parameters.Add("NoItemID", ItemTransferID);
                        ...
                        alert_box.Visible = true;
                        alert_box.Controls.Add(new LiteralControl("Update Data Material Transfer Successfully"));
                        alert_box.Attributes.Add("class", "alert alert-success alert-dismissible");
       ...
       //   {
        ...
        //    {
            if (IS_DELETE_ACTIVE == "Enable")
            {
                ...
                string delete_user = " delete from PF_MATERIAL_TRANSACTION where MAT_TRANS_ID  = '" + MatTransactionForID + "'"; 
