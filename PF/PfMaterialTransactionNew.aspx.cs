...
{
    public partial class PfMaterialTransactionNew : System.Web.UI.Page
    {
        ...
        public void BtnAdd_Click(object sender, EventArgs e)
        {
         ...
         //     { 
                ...
                if (TransactionForID == 1) {
                    ...
                    string insert_trans = "insert into  PF_MATERIAL_TRANSACTION (MAT_TRANS_ID, TRANSACTION_FOR_ID, ITEM_ID, ITEM_WEIGHT, ITEM_TRANSFER_ID, ENTRY_DATE, REMARKS, CREATE_DATE, C_USER_ID, IS_ACTIVE) values  ('MT-' || LPAD(:NoTransactionID, 6, '0') , :NoTransactionForID, :NoItemID, :NoItemWeight, :NoItemTransferID, TO_DATE(:EntryDate, 'DD/MM/YYYY'), :TextRemarks, TO_DATE(:c_date, 'DD-MM-YYYY HH:MI:SS AM'), :NoCuserID, :TextIsActive)";
                    ...
                    objPrm[7] = cmdi.Parameters.Add("c_date", c_date);
                ...
                else if(TransactionForID == 3)
                {
                    ...
                    cmdu.Dispose();
                        ...
                        string makeTransSQL = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemTransferID + "'  ";
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
          if (IS_PRINT_ACTIVE == "Enable")
            ...
            string HtmlString = "";
            string makeSQL = " SELECT WMT.MAT_TRANS_ID, WMT.TRANSACTION_FOR_ID, NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WMT.ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER, WMT.REMARKS, TO_CHAR(WMT.ENTRY_DATE,'dd-MON-yyyy') AS ENTRY_DATE, WMT.CREATE_DATE, WMT.UPDATE_DATE, WMT.IS_ACTIVE, WMT.IS_PRINT, TO_CHAR(WMT.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM  PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE  WMT.MAT_TRANS_ID = '" + TransactionForID + "' ";
            ...
            HtmlString += "<div style='float:left;width:825px;text-align:center;font-family:Times New Roman,Times, serif;font-size:16px;font-weight:700;' >Certificate Of " + TransanctionFor + "</div> ";
            ...
            HtmlString += "<tr>";
                    if (dt.Rows[i]["TRANSACTION_FOR_ID"].ToString() == "3") {
                        HtmlString += "<th><span style='font-family:Times New Roman;'>" + TransanctionFor + " From </span></th>"; } else {
        ...
        public void DisplaySummary()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT  NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, SUM(WMT.ITEM_WEIGHT) AS ITEM_WEIGHT, WIT.ITEM_NAME AS ITEM_NAME_TRANSFER FROM PF_MATERIAL_TRANSACTION WMT LEFT JOIN NRC_MATERIAL_TRANSACTION_FOR NMTF ON NMTF.TRANSACTION_FOR_ID = WMT.TRANSACTION_FOR_ID LEFT JOIN PF_ITEM WI ON WI.ITEM_ID = WMT.ITEM_ID LEFT JOIN PF_ITEM WIT ON WIT.ITEM_ID = WMT.ITEM_TRANSFER_ID WHERE to_char(WMT.ENTRY_DATE, 'mm/yyyy') = '" + MonthYear + "' GROUP BY NMTF.TRANSACTION_FOR_NAME, WI.ITEM_NAME, WIT.ITEM_NAME ORDER BY WI.ITEM_NAME   ";
                }
                else
                {
        ...
        //   {
            if (IS_EDIT_ACTIVE == "Enable")
            {
                ...
                if (TransactionForID == 3) { ItemTransferID = Convert.ToInt32(DropDownItemTransferID.Text); } else { ItemTransferID = 0; }
                string ISActive = CheckIsActive.Checked ? "Enable" : "Disable";
                ...
                // get old purchase item weight
                    ...
                    string makeSQLCheckRM = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "'  ";
                ...
                if (TransactionForID == 1)
                {
                    if (ItemWeight <= FinalStock)
                    {
                        ...
                        string update_inven_old = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_OUT_WT = :NoStockOut, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                        ...
                        string makeSQLUpdateRM = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "'  ";
                ...
                else if (TransactionForID == 2)
                {
                    ...
                    string update_inven_old = "update  PF_RM_STOCK_INVENTORY_MASTER  set STOCK_IN_WT = :NoStockIn, FINAL_STOCK_WT = :NoFinalStock, UPDATE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), U_USER_ID = :NoCuserID  where ITEM_ID = :NoItemID  ";
                    cmdl = new OracleCommand(update_inven_old, conn);
       ...
       //   {
        ...
        //    {
            if (IS_DELETE_ACTIVE == "Enable")
            {
                ...
                double FinalStockCheck = 0.00;
                string makeSQLCheckRM = " select * from PF_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemID + "'  ";
                cmdl = new OracleCommand(makeSQLCheckRM);
                oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                dt = new DataTable();
                oradata.Fill(dt);
