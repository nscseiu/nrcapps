...
{
    public partial class WpExpSalesInvoice : System.Web.UI.Page
    {
            ...
            if (Session["USER_NAME"] != null)
            {
                ...
                if (IS_PAGE_ACTIVE == "Enable")
                {
                  ...
                  //      DropDownInvPrintFor.Items.Insert(0, new ListItem("Select Print For", "0"));
                        ...
                        string makePartySQL = " SELECT PARTY_ID,  PARTY_ID || ' - ' || PARTY_NAME || ' - ' || PARTY_VAT_NO AS PARTY_NAME  FROM WP_PARTY WHERE IS_ACTIVE = 'Enable' ORDER BY PARTY_NAME ASC";
                        ...
                        string makePageSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Driver Name -' || DRIVER_NAME  AS PARTY_NAME FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO WHERE PEWC.EXPORT_INVOICE_NO IS NULL AND PEWCI.IS_ACTIVE_PRICING = 'Enable' GROUP BY PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO || ' - ' || PEWC.WB_SLIP_NO || ', Driver Name -' || DRIVER_NAME ORDER BY PEWC.WB_SLIP_NO ASC";
                ...
                if (IS_ADD_ACTIVE == "Enable")
                {
                    ...
                    foreach (ListItem li in DropDownSlipNoEx.Items)
                    {
                        ...
                        if (li.Selected == true)
                        {
                            ...
                            string update_ex_invoice = " update  WP_EXPORT_WBSLIP_CON set EXPORT_INVOICE_NO =:NoExportInvoiceNo, IS_CONFIRM_CHECK =:IsConfirm,  IS_CONFIRM_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_CONFIRM_ID = :NoCuserID  where WB_SLIP_NO =: NoWbSlipNo ";
          ...
          try
               {  
                if (IS_ADD_ACTIVE == "Enable")
                {
                    ...
                    string get_id = "select WP_EXPORT_CONTRACT_NO_ID_SEQ.nextval from dual";
                    cmdu = new OracleCommand(get_id, conn);
                    int newID = Int32.Parse(cmdu.ExecuteScalar().ToString()); 
                     
                    string insert_ex_sales = "insert into WP_EXPORT_CONTRACT_NO (EXPORT_CONTRACT_NO_ID, CONTRACT_NO, CONTRACT_NO_SERIAL, CONTRACT_DATE, EXPORT_INVOICE_NO, CREATE_DATE, C_USER_ID) VALUES ( :NoContractNoID,  :TextContractNo, :TextContractNoSerial, TO_DATE(:ContractDate, 'DD/MM/YYYY'), :TextExportInvoiceNo, TO_DATE(:c_date, 'DD-MM-YYYY HH:MI:SS AM'),  :NoCuserID)";
                    cmdi = new OracleCommand(insert_ex_sales, conn); 
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string makeContractSQL = " SELECT CONTRACT_NO, CONTRACT_NO_SERIAL, TO_CHAR(CONTRACT_DATE, 'DD-MON-YYYY') AS CONTRACT_DATE FROM WP_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '" + TextExportInvoiceNo.Text + "' "; 
            ...
            string makeContainerSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, NCS.CONTAINER_SIZE_INWORDS FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID LEFT JOIN WP_EXPORT_SALES_MASTER PESM ON PESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO  WHERE PESM.EXPORT_INVOICE_NO = '" + TextExportInvoiceNo.Text + "' ORDER BY PEWC.CONTAINER_NO ASC "; 
            ...
            { 
                ...
                HtmlString += "<div style='float:left;width:792px;height:auto;margin-top:50px;font-family: Arial, Courier, Lucida Sans Typewriter, Lucida Typewriter, monospace; font-size: 12px; font-style: normal; font-variant: normal; font-weight: 400; line-height: 16px;'> ";
              
                HtmlString += "<div style='float:left;width:750px;height:20px; margin:0 5px 0 40px;text-align:center;border-bottom: black solid 1px;' ><span style='font-family:Arial,Times, serif;font-size:17px;font-weight:700;'>INVOICE</span></div> ";
                  
                HtmlString += "<div style='float:left;width:400px;height:40px; margin:0 0 0 40px;'><span style='font-family:Arial,Times, serif;font-size:15px;'>INVOICE NO. " + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + " </span></div> ";
                HtmlString += "<div style='float:left;width:350px;height:40px;text-align:right;'><span style='font-family:Arial,Times, serif;font-size:15px;'>DATE:  " + dt.Rows[i]["INVOICE_DATE"].ToString() + "</span></div> ";
            ...
            HtmlString += "<div style='float:left;width:740px;height:auto;margin:0 0 0 40px;-webkit-border-top-left-radius:10px;-webkit-border-top-right-radius:10px;font-family: Arial Narrow, Courier, Lucida Sans Typewriter, Lucida Typewriter, monospace; font-size:12px;'> ";

            HtmlString += "<table cellpadding='3px' cellspacing='0' style='font-size: 12px;' >";
            HtmlString += "<th style='border:black solid 1px; -webkit-border-top-left-radius:10px;width:85px;'>MARKS & NO. OF CONTAINER (S)</th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;'>SIZE</span></th> ";
            HtmlString += "<th style='border-top:black solid 1px;border-bottom:black solid 1px;border-right:black solid 1px;width:360px;'>DESCRIPTION OF MATERIALS</th> ";
            ...
            HtmlString += "<tr valign='top'> ";
               ...
               for (int i=0; i< RowCount; i++) {
                ...
                HtmlString += "<td  style='border-top:black solid 1px;border-right:black solid 1px;border-bottom:black solid 1px;border-left:black solid 1px;text-align:right;'> " + string.Format("{0:n2}", TotalInvoiceAmt) + " </td>";
                HtmlString += "</tr> ";
            ...
            string update_user = "update  WP_EXPORT_SALES_MASTER  set IS_PRINT = :TextIsPrint, PRINT_DATE  = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), P_USER_ID = :NoCuserID where EXPORT_SALES_ID = :NoExportSalesID ";
       ...
       //    {
            ...
            { 
                ...
                string makeSlipSQL = " SELECT PEWC.EXPORT_INVOICE_NO FROM WP_EXPORT_SALES_MASTER PESM LEFT JOIN WP_EXPORT_WBSLIP_CON PEWC ON PEWC.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO WHERE  PESM.EXPORT_SALES_ID = '" + ExportSalesID + "'";
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    ...
                    string update_user = "update  WP_EXPORT_WBSLIP_CON set EXPORT_INVOICE_NO =:NoExportInvoiceNo, IS_CONFIRM_CHECK =:IsConfirm,  IS_CONFIRM_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_CONFIRM_ID = :NoCuserID  where EXPORT_INVOICE_NO ='" + ExInvoiceNo + "' ";
                    cmdi = new OracleCommand(update_user, conn);
        ...
        protected void DeleteContractClick(object sender, EventArgs e)
        {
            try
            {
                if (IS_DELETE_ACTIVE == "Enable")
                {
                    ...
                    string delete_ex_contract = " delete from WP_EXPORT_CONTRACT_NO where EXPORT_CONTRACT_NO_ID  = '" + USER_DATA_ID + "'"; 
                    cmdi = new OracleCommand(delete_ex_contract, conn); 
                    cmdi.ExecuteNonQuery();
                    cmdi.Parameters.Clear();
                    cmdi.Dispose();
        ...
        { 
            ...
            string makeSQL = " SELECT EXPORT_SALES_ID, EXPORT_INVOICE_NO, PAY_TERMS_ID,  SHIPPING_INCO_ID, TRADING_VESSEL_ID, TRADING_VESSEL_CODE, TO_CHAR(SHIPMENT_DATE,'dd/mm/yyyy') AS SHIPMENT_DATE, TO_CHAR(INVOICE_DATE,'dd/mm/yyyy') AS INVOICE_DATE, PARTY_ID, PARTY_ADDRESS_ID, SHIPMENT_FROM_TO_ID, TO_CHAR(SHIPMENT_DATE,'dd/mm/yyyy') AS SHIPMENT_DATE, CREATE_DATE, UPDATE_DATE, IS_ACTIVE, IS_PRINT, PRINT_DATE FROM WP_EXPORT_SALES_MASTER WHERE EXPORT_SALES_ID = '" + USER_DATA_ID + "'";
             ...
             { 
                ...
                DataTable dtSlipNo = new DataTable();
                DataSet dsp = new DataSet();
                string makePageSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Driver Name -' || DRIVER_NAME  AS PARTY_NAME FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO WHERE  PEWCI.IS_ACTIVE_PRICING = 'Enable' AND PEWC.EXPORT_INVOICE_NO = '" + ds.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "'   OR PEWC.EXPORT_INVOICE_NO IS NULL  GROUP BY PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO || ' - ' || PEWC.WB_SLIP_NO || ', Driver Name -' || DRIVER_NAME ORDER BY PEWC.WB_SLIP_NO ASC"; // AND(PEWC.IS_CONFIRM_CHECK IS NULL AND PEWC.EXPORT_INVOICE_NO IS NULL)  PEWC.EXPORT_INVOICE_NO IS NULL AND
                ...
                string makePartyAddIDSQL = " SELECT PARTY_ADDRESS_ID, PARTY_ADD_1 || ', ' || PARTY_ADD_2 AS PARTY_ADDRESS FROM WP_PARTY_ADDRESS WHERE PARTY_ID = '" + ds.Rows[i]["PARTY_ID"].ToString() + "' ORDER BY PARTY_ADDRESS_ID ASC";
             ...
             string makeSlipSQL = " SELECT PEWC.WB_SLIP_NO, PEWC.CONTAINER_NO ||' - ' ||  PEWC.WB_SLIP_NO || ', Driver Name -' || PEWC.DRIVER_NAME AS PARTY_NAME FROM WP_EXPORT_SALES_MASTER PESM LEFT JOIN WP_EXPORT_WBSLIP_CON PEWC ON PEWC.EXPORT_INVOICE_NO = PESM.EXPORT_INVOICE_NO WHERE PESM.EXPORT_SALES_ID = '" + USER_DATA_ID + "' ORDER BY PEWC.WB_SLIP_NO ASC  ";
        ...
        public void DisplayContract()
           {
            ...
            string makeSQL = " SELECT * FROM WP_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '"+ TextExportInvoiceNo.Text  + "' ";
        ...
        public void TextPartyAddress_TextChanged(object sender, EventArgs e)
        {
            DataTable dtPartyAddressID = new DataTable();
            DataSet dsa = new DataSet();
            string makePartyAddIDSQL = " SELECT PARTY_ADDRESS_ID, PARTY_ADD_1 || ', ' || PARTY_ADD_2 AS PARTY_ADDRESS FROM WP_PARTY_ADDRESS WHERE PARTY_ID = '" + DropDownPartyID.Text + "' ORDER BY PARTY_ADDRESS_ID ASC";
            dsa = ExecuteBySqlString(makePartyAddIDSQL);
            dtPartyAddressID = (DataTable)dsa.Tables[0];
            DropDownPartyAddressID.DataSource = dtPartyAddressID;
            DropDownPartyAddressID.DataValueField = "PARTY_ADDRESS_ID";
        ...
        public void TextExportInvoiceNo_TextChanged(object sender, EventArgs e)
        {
            ...
            if (ExportInvoiceNo != null)
            {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandText = "select EXPORT_INVOICE_NO from WP_EXPORT_SALES_MASTER where EXPORT_INVOICE_NO = '" + ExportInvoiceNo + "'";
        ...
        protected void BtnReport_Click(object sender, EventArgs e)
        {
            if (IS_REPORT_ACTIVE == "Enable")
            {
                ...
                string HtmlString = "";
                string makeSQL = " SELECT  PESM.EXPORT_SALES_ID, PESM.EXPORT_INVOICE_NO, NPT.PAY_TERMS_NAME, NSI.SHIPPING_INCO_NAME, NTV.TRADING_VESSEL_NAME, PESM.TRADING_VESSEL_CODE,  TO_CHAR(PESM.SHIPMENT_DATE,'dd-MON-yyyy') AS SHIPMENT_DATE, TO_CHAR(PESM.INVOICE_DATE,'dd-mm-yyyy') AS INVOICE_DATE,  NSLF.SHIPMENT_LOC_NAME AS SHIPMENT_FROM_NAME, NSLT.SHIPMENT_LOC_NAME AS SHIPMENT_TO_NAME, PESM.CREATE_DATE, PESM.UPDATE_DATE, PESM.IS_ACTIVE, PESM.IS_PRINT, PESM.PRINT_DATE, PP.PARTY_NAME, PP.PARTY_ADD_1, PP.PARTY_ADD_2, PP.PARTY_VAT_NO FROM WP_EXPORT_SALES_MASTER PESM LEFT JOIN NRC_PAYMENT_TERMS NPT ON NPT.PAY_TERMS_ID = PESM.PAY_TERMS_ID LEFT JOIN NRC_SHIPPING_INCOTERMS NSI ON NSI.SHIPPING_INCO_ID = PESM.SHIPPING_INCO_ID LEFT JOIN NRC_TRADING_VESSEL NTV ON NTV.TRADING_VESSEL_ID = PESM.TRADING_VESSEL_ID  LEFT JOIN NRC_SHIPMENT_FROM_TO NSRT ON NSRT.SHIPMENT_FT_ID = PESM.SHIPMENT_FROM_TO_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLF ON NSLF.SHIPMENT_LOC_ID = NSRT.SHIPMENT_FROM_ID LEFT JOIN NRC_SHIPMENT_LOCATION NSLT ON NSLT.SHIPMENT_LOC_ID = NSRT.SHIPMENT_TO_ID  LEFT JOIN WP_PARTY PP ON PP.PARTY_ID = PESM.PARTY_ID  WHERE TO_CHAR(PESM.SHIPMENT_DATE ,'mm-yyyy') = '" + ShipmentDate + "' ORDER BY PESM.EXPORT_INVOICE_NO ASC ";
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    string makeContractSQL = " SELECT CONTRACT_NO || '-' || CONTRACT_NO_SERIAL AS CONTRACT_NO, CONTRACT_NO_SERIAL, TO_CHAR(CONTRACT_DATE, 'DD-MON-YYYY') AS CONTRACT_DATE FROM WP_EXPORT_CONTRACT_NO WHERE EXPORT_INVOICE_NO = '" + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "' ";
                    cmdl = new OracleCommand(makeContractSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    dc = new DataTable();
                    oradata.Fill(dc);
                    ...
                    string makeContainerSQL = " SELECT  count(PEWC.CONTAINER_NO) CONTAINER_NO, PI.ITEM_NAME AS ITEM_NAME , SUM((nvl(PEWCI.ITEM_WEIGHT,0) / 1000)) AS ITEM_WEIGHT, nvl(PEWCI.MAT_PRICE_PER_MT,0) AS MAT_PRICE_PER_MT, nvl(SUM(PEWCI.MATERIAL_AMOUNT),0) AS MATERIAL_AMOUNT, SUM((PEWCI.CURRENCY_RATE * PEWCI.MAT_PRICE_PER_MT)) AS MAT_CONVERSION_PRICE_PER_MT, SUM(PEWCI.MATERIAL_CONVERSION_AMOUNT) AS MATERIAL_CONVERSION_AMOUNT FROM WP_EXPORT_SALES_MASTER PESM LEFT JOIN WP_EXPORT_WBSLIP_CON PEWC ON PESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PEWCI.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID LEFT JOIN NRC_CURRENCY_RATE NCR ON NCR.CURRENCY_RATE_ID = PEWCI.CURRENCY_RATE_ID LEFT JOIN NRC_CURRENCY NCS ON NCS.CURRENCY_ID = NCR.SOURCE_CURRENCY_ID LEFT JOIN NRC_CURRENCY NCT ON NCT.CURRENCY_ID = NCR.TARGET_CURRENCY_ID WHERE PESM.EXPORT_INVOICE_NO = '" + dt.Rows[i]["EXPORT_INVOICE_NO"].ToString() + "' GROUP BY PI.ITEM_ID, PI.ITEM_NAME, PEWCI.MAT_PRICE_PER_MT  ORDER BY PI.ITEM_ID ";
                    cmdl = new OracleCommand(makeContainerSQL);
                    oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                    ds = new DataTable();
                    oradata.Fill(ds);
