...
{
    public partial class WpExpShipmentCmo : System.Web.UI.Page
    {
                ...
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    foreach (GridViewRow gridRow in GridView1.Rows)
                    {
                        ...
                        { 
                            ...
                            string makeSQLPro = " select ITEM_ID, ITEM_WEIGHT from WP_EXPORT_WBSLIP_CON_ITEM where EXP_WBCON_ITEM_ID  = '" + gridRow.Cells[0].Text + "'  ";
                            ...
                            // check inventory RM
                            string makeSQL = " select * from WP_RM_STOCK_INVENTORY_MASTER where ITEM_ID  = '" + ItemIdOld + "' ";
                            cmdl = new OracleCommand(makeSQL);
                            oradata = new OracleDataAdapter(cmdl.CommandText, conn);
                            dt = new DataTable();
                            oradata.Fill(dt);
                            ...
                            { 
                                ...
                                string update_ex_invoice = " update  WP_EXPORT_WBSLIP_CON_ITEM set IS_INVENTORY_STATUS =:IsConfirm,  IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM') , IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where EXP_WBCON_ITEM_ID =:NoWbItemID ";
        ...
        public void Display()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT PEWCI.EXP_WBCON_ITEM_ID, PEWC.WB_SLIP_NO, PP.PARTY_NAME, PEWC.DRIVER_NAME, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.REL_ORDER_NO, PEWC.IS_EDIT, PEWCI.EXP_WBCON_ITEM_ID, PI.ITEM_NAME, PEWC.ITEM_WEIGHT_WB, PEWC.TARE_WEIGHT, PEWCI.ITEM_WEIGHT, PEWCI.ITEM_BALES, PEWC.IS_ACTIVE, PEWCI.IS_INVENTORY_STATUS, PEWCI.IS_ACTIVE_PRICING, PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE, PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN WP_EXPORT_SALES_MASTER WESM ON WESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO LEFT JOIN WP_PARTY PP ON PP.PARTY_ID = WESM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PEWCI.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE PEWCI.IS_INVENTORY_STATUS = 'Transit' AND PEWC.IS_CONFIRM_CHECK = 'Complete' ORDER BY PEWC.WB_SLIP_NO desc, PEWCI.ITEM_ID ASC "; // to_char(PEWC.DISPATCH_DATE, 'mm/yyyy') = '" + MonthYear + "' AND
                }
                else
                {
                    makeSQL = " SELECT PEWCI.EXP_WBCON_ITEM_ID, PEWC.WB_SLIP_NO, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.SEAL_NO, PEWC.REL_ORDER_NO, PP.PARTY_NAME,  PI.ITEM_NAME, PSI.SUB_ITEM_NAME, PEWC.ITEM_WEIGHT_WB,  PEWC.ITEM_WEIGHT,  NPL.PACKING_NAME, PEPH.NUMBER_OF_PACK, PEPH.PACK_PER_WEIGHT,  PEPH.TOTAL_WEIGHT, PS.SALESMAN_NAME, PEWC.IS_ACTIVE ,  PEWC.IS_INVENTORY_STATUS,  PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE,  PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM PF_EXPORT_WBSLIP_CON PEWC LEFT JOIN PF_EXPORT_PACKING_HISTORY PEPH ON PEPH.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN NRC_PACKING_LIST NPL ON NPL.PACKING_ID = PEPH.PACKING_ID LEFT JOIN PF_PARTY PP ON PP.PARTY_ID = PEWC.PARTY_ID LEFT JOIN PF_SALESMAN PS ON PS.SALESMAN_ID = PEWC.SALESMAN_ID LEFT JOIN PF_ITEM PI ON PI.ITEM_ID = PEWC.ITEM_ID LEFT JOIN PF_SUB_ITEM PSI ON PSI.SUB_ITEM_ID = PEWC.SUB_ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE PEWC.IS_INVENTORY_STATUS = 'Transit' AND PEWC.IS_CONFIRM_CHECK = 'Complete' AND (PEWC.CONTAINER_NO like '" + txtSearchGrideView1.Text + "%' OR to_char(PEWC.DISPATCH_DATE, 'dd/mm/yyyy')  like '" + txtSearchGrideView1.Text + "%' or  PEWC.CONTAINER_NO like '" + txtSearchGrideView1.Text + "%')  ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  ";  
        ...
        public void DisplayComplete()
        {
            if (IS_VIEW_ACTIVE == "Enable")
            {
                ...
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PP.PARTY_NAME, PEWC.DRIVER_NAME, PEWCI.IS_SHIPMENT_COMPLETE_DATE, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.REL_ORDER_NO, PEWC.IS_EDIT, PEWCI.EXP_WBCON_ITEM_ID, PI.ITEM_NAME, PEWC.ITEM_WEIGHT_WB, PEWC.TARE_WEIGHT, PEWCI.ITEM_WEIGHT, PEWCI.ITEM_BALES, PEWC.IS_ACTIVE, PEWCI.IS_INVENTORY_STATUS, PEWCI.IS_ACTIVE_PRICING, PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE, PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN WP_EXPORT_SALES_MASTER WESM ON WESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO LEFT JOIN WP_PARTY PP ON PP.PARTY_ID = WESM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PEWCI.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE PEWCI.IS_INVENTORY_STATUS = 'Complete' AND PEWC.IS_CONFIRM_CHECK = 'Complete' ORDER BY PEWC.WB_SLIP_NO desc, PEWCI.ITEM_ID ASC   ";  // to_char(PEWC.DISPATCH_DATE, 'mm/yyyy') = '" + MonthYear + "' AND
                }
                else
                {
                    makeSQL = " SELECT PEWC.WB_SLIP_NO, PP.PARTY_NAME, PEWC.DRIVER_NAME, PEWCI.IS_SHIPMENT_COMPLETE_DATE, PEWC.DISPATCH_DATE, PEWC.CONTAINER_NO, NCS.CONTAINER_SIZE, PEWC.REL_ORDER_NO, PEWC.IS_EDIT, PEWCI.EXP_WBCON_ITEM_ID, PI.ITEM_NAME, PEWC.ITEM_WEIGHT_WB, PEWC.TARE_WEIGHT, PEWCI.ITEM_WEIGHT, PEWCI.ITEM_BALES, PEWC.IS_ACTIVE, PEWCI.IS_INVENTORY_STATUS, PEWCI.IS_ACTIVE_PRICING, PEWC.CREATE_DATE, PEWC.UPDATE_DATE, PEWC.IS_PRINT,  PEWC.PRINT_DATE, PEWC.IS_CONFIRM_CHECK, PEWC.EXPORT_INVOICE_NO FROM WP_EXPORT_WBSLIP_CON PEWC LEFT JOIN WP_EXPORT_WBSLIP_CON_ITEM PEWCI ON PEWCI.WB_SLIP_NO = PEWC.WB_SLIP_NO LEFT JOIN WP_EXPORT_SALES_MASTER WESM ON WESM.EXPORT_INVOICE_NO = PEWC.EXPORT_INVOICE_NO LEFT JOIN WP_PARTY PP ON PP.PARTY_ID = WESM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PEWCI.ITEM_ID LEFT JOIN NRC_CONTAINER_SIZE NCS ON NCS.CONTAINER_SIZE_ID = PEWC.CONTAINER_SIZE_ID WHERE PEWCI.IS_INVENTORY_STATUS = 'Complete' AND PEWC.IS_CONFIRM_CHECK = 'Complete' AND (PEWC.CONTAINER_NO like '" + txtSearchGrideView2.Text + "%' OR to_char(PEWC.DISPATCH_DATE, 'dd/mm/yyyy')  like '" + txtSearchGrideView2.Text + "%' or  PEWC.WB_SLIP_NO like '" + txtSearchGrideView2.Text + "%')  ORDER BY PEWC.WB_SLIP_NO desc, PEWC.ITEM_ID ASC  ";
        ...
        protected void linkSelectClick(object sender, EventArgs e)
        {
            ...
            string makeSQL = "  SELECT WB_SLIP_NO, EXP_WBCON_ITEM_ID, TO_CHAR(IS_SHIPMENT_COMPLETE_DATE, 'DD/MM/YYYY') AS IS_SHIPMENT_COMPLETE_DATE FROM WP_EXPORT_WBSLIP_CON_ITEM  WHERE EXP_WBCON_ITEM_ID = '" + USER_DATA_ID + "'";
        ...
        public void BtnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IS_EDIT_ACTIVE == "Enable")
                {
                    ...
                    // update data  
                    string update_ex_invoice = " update  WP_EXPORT_WBSLIP_CON_ITEM set IS_SHIPMENT_COMPLETE_DATE = TO_DATE(:EntryDate, 'DD/MM/YYYY'), IS_SHIPMENT_COMPLETE_C_ID = :NoCuserID  where EXP_WBCON_ITEM_ID =: NoWbSlipNo ";
                    cmdi = new OracleCommand(update_ex_invoice, conn);
