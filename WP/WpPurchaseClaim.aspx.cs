...
{
    public partial class WpPurchaseClaim : System.Web.UI.Page
    {
            ...
            if (Session["USER_NAME"] != null)
            {
                ...
                if (IS_PAGE_ACTIVE == "Enable")
                {
                   ...
                   //     DropDownPaymentTypeID.Items.Insert(0, new ListItem("Select User Role", "0"));
                        ...
                        string makePageSQL = " SELECT PPM.PURCHASE_ID || '-' || PPM.TOTAL_AMOUNT AS PURCHASE_ID, PPM.SLIP_NO || ' - ' || PC.PARTY_ID || ' - ' || PC.PARTY_NAME || ', Item -' || PI.ITEM_NAME || ', Weight- ' || PPM.ITEM_WEIGHT || ', Amount- ' || TO_CHAR(PPM.TOTAL_AMOUNT, '999,999,999.99') AS PARTY_NAME FROM WP_PURCHASE_MASTER PPM  LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PPM.ITEM_ID  WHERE PPM.CLAIM_NO IS NULL  ORDER BY  PPM.SLIP_NO ASC";
           ...
           {  
                if (IS_ADD_ACTIVE == "Enable")
                {
                  ...
                  //  int ClaimNoFrom = Convert.ToInt32(TextClaimNo.Text);
                    ...
                    string get_pur_claim_id = "select WP_PURCHASE_CLAIM_ID_SEQ.nextval from dual";
                    cmdu = new OracleCommand(get_pur_claim_id, conn);
                    int newPurClaimID = Int32.Parse(cmdu.ExecuteScalar().ToString());
                     
                    string get_pur_claim_no = "select WP_PURCHASE_CLAIMNO_SEQ.nextval from dual";
                    ...
                    foreach (ListItem li in DropDownSlipNoWp.Items)
                    {
                        ...
                        if (li.Selected == true)
                        {
                            ...
                            string update_purchase = " update  WP_PURCHASE_MASTER set CLAIM_NO =:NoClaimNo  where PURCHASE_ID =: NoSlipNo ";
                    ...
                    string insert_pur_claim = "insert into WP_PURCHASE_CLAIM (PURCHASE_CLAIM_ID, CLAIM_NO, EMP_ID, CLAIM_DATE, CLAIM_FOR_MONTH, PAYMENT_TYPE_ID, TOTAL_AMOUNT, CREATE_DATE, C_USER_ID, IS_ACTIVE, IS_CHECK, IS_OBJ_QUERY) VALUES ( :NoPurClaimID, :NoClaimNo, :NoEmpID, TO_DATE(:ClaimDate, 'DD/MM/YYYY'), TO_DATE(:ClaimForMonth, 'DD/MM/YYYY'), :NoPaymentTypeID, :NoTotalAmount, TO_DATE(:c_date, 'DD-MM-YYYY HH:MI:SS AM'),  :NoCuserID, :TextIsActive, :TextIsCheck, :TextIsObjQuery)";
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string makeSQL = " SELECT PPC.CLAIM_NO, TO_CHAR(TO_DATE(CLAIM_DATE),'dd/MON/YYYY') AS CLAIM_DATE, TO_CHAR(TO_DATE(CLAIM_FOR_MONTH),'MON/YYYY') AS CLAIM_MONTH, HE.EMP_FNAME || ' '|| HE.EMP_LNAME AS FULL_NAME,  PP.PARTY_NAME, PPM.SLIP_NO, PPM.ITEM_AMOUNT+nvl(PPM.VAT_AMOUNT,0) AS ITEM_AMOUNT FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN WP_PARTY PP ON PP.PARTY_ID = PPM.PARTY_ID  LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID WHERE PPC.CLAIM_NO = '" + ClaimNo + "' ORDER BY PPM.SLIP_NO ASC";
            ...
            { 
                ...
                HtmlString += "<div style='float:left;width:210px;height:40px;font-weight:700;size:13px;'><span style='font-family:Times New Roman,Times, serif;'>Division:</span><span style='color:#5447e3;'> Waste Paper</span></div>  ";
            ...
            string update_user = "update  WP_PURCHASE_CLAIM  set IS_PRINT = :TextIsPrint, PRINT_DATE  = TO_DATE(:u_date, 'DD-MM-YYYY HH:MI:SS AM'), P_USER_ID = :NoCuserID where CLAIM_NO = :NoClaimNo ";
        ...
        //    {
            ...
            { 
                ...
                string ClaimForMonths = ClaimForMonthsNew.ToString("dd-MM-yyyy");
                    ...
                    string makeSlipSQL = " SELECT PPC.CLAIM_NO, PPM.SLIP_NO FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO WHERE  PPC.PURCHASE_CLAIM_ID = '" + PurchaseClaimID + "'";
                ...
                RowCount = dt.Rows.Count;
                    ...
                    for (int i = 0; i < RowCount; i++)
                    {
                        ...
                        string update_user = "update  WP_PURCHASE_MASTER set CLAIM_NO =:NoClaimID  where CLAIM_NO ='" + ClaimNo + "' ";
                        cmdi = new OracleCommand(update_user, conn);
                ...
                DataTable dtSlipNo = new DataTable();
                DataSet dsp = new DataSet();
                string makePageSQL = " SELECT PPM.SLIP_NO, PPM.SLIP_NO || ' - ' || PC.PARTY_ID || ' - ' || PC.PARTY_NAME AS PARTY_NAME FROM WP_PURCHASE_MASTER PPM  LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID  WHERE PPM.CLAIM_NO IS NULL  ORDER BY  PPM.SLIP_NO ASC";
                dsp = ExecuteBySqlString(makePageSQL);
                dtSlipNo = (DataTable)dsp.Tables[0];
                DropDownSlipNoWp.DataSource = dtSlipNo;
        ...
        protected void BtnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (IS_DELETE_ACTIVE == "Enable")
                {
                    ...
                    string delete_pur_claims = " delete from WP_PURCHASE_CLAIM where PURCHASE_CLAIM_ID  = '" + PurchaseClaimID + "'";
        ...
        { 
             ...
             DataTable dtSlipNo = new DataTable();
             DataSet dsp = new DataSet();
             string makePageSQL = " SELECT PPM.PURCHASE_ID || '-' || PPM.TOTAL_AMOUNT AS PURCHASE_ID, PPM.SLIP_NO || ' - ' || PC.PARTY_ID || ' - ' || PC.PARTY_NAME || ', Item -' || PI.ITEM_NAME || ', Weight- ' || PPM.ITEM_WEIGHT || ', Amount- ' || TO_CHAR(PPM.TOTAL_AMOUNT, '999,999,999.99') AS PARTY_NAME FROM WP_PURCHASE_MASTER PPM  LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PPM.ITEM_ID  WHERE (PPM.CLAIM_NO IS NULL OR PPM.CLAIM_NO = '" + USER_DATA_ID + "' ) ORDER BY  PPM.SLIP_NO ASC"; //  
             dsp = ExecuteBySqlString(makePageSQL);
             dtSlipNo = (DataTable)dsp.Tables[0];
             DropDownSlipNoWp.DataSource = dtSlipNo;
             DropDownSlipNoWp.DataValueField = "PURCHASE_ID";
             ...
             string makeSQL = " SELECT PURCHASE_CLAIM_ID, CLAIM_NO, EMP_ID, TO_CHAR(TO_DATE(CLAIM_DATE),'dd/mm/yyyy') AS CLAIM_DATE, TO_CHAR(TO_DATE(CLAIM_FOR_MONTH),'mm/yyyy') CLAIM_FOR_MONTH, PAYMENT_TYPE_ID, TOTAL_AMOUNT, IS_ACTIVE FROM WP_PURCHASE_CLAIM where CLAIM_NO = '" + USER_DATA_ID + "'";
             ...
             string makeSlipSQL = " SELECT PPC.CLAIM_NO, PPM.PURCHASE_ID || '-' || PPM.TOTAL_AMOUNT AS PURCHASE_ID FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO WHERE PPC.IS_CHECK = 'Incomplete' AND  PPC.CLAIM_NO = '" + USER_DATA_ID + "' ORDER BY PPM.SLIP_NO ASC";
        ...
        public void Display()
        {
            ...
            {
                makeSQL = " SELECT PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME AS EMP_NAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, PPM.SLIP_NO, PC.PARTY_ID || ' - ' || PC.PARTY_NAME AS PARTY_NAME, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE  FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID LEFT JOIN NRC_PAYMENT_TYPE NPT ON NPT.PAYMENT_TYPE_ID = PPC.PAYMENT_TYPE_ID LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID WHERE PPC.IS_CHECK = 'Incomplete' ORDER BY PPC.CLAIM_NO DESC, PPM.CLAIM_NO ASC "; //WHERE PPC.IS_CHECK = 'Incomplete'
            }
            else
            {
                makeSQL = " SELECT PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME AS EMP_NAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, PPM.SLIP_NO, PC.PARTY_ID || ' - ' || PC.PARTY_NAME AS PARTY_NAME, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID LEFT JOIN NRC_PAYMENT_TYPE NPT ON NPT.PAYMENT_TYPE_ID = PPC.PAYMENT_TYPE_ID LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID WHERE PPC.CLAIM_NO like '" + txtSearchUser.Text + "%' or  HE.EMP_FNAME like '" + txtSearchUser.Text + "%'  ORDER BY PPC.CLAIM_NO DESC, PPM.SLIP_NO ASC ";
                alert_box.Visible = false;
            }
        ...
        public void Display()
        {
            ...
            if (txtSearchUser.Text == "")
            {
                 makeSQL = " SELECT PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME AS EMP_NAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, count(PPM.SLIP_NO) as SLIP_NO, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID LEFT JOIN NRC_PAYMENT_TYPE NPT ON NPT.PAYMENT_TYPE_ID = PPC.PAYMENT_TYPE_ID LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID WHERE PPC.IS_CHECK = 'Incomplete' AND PPC.IS_ACTIVE = 'Enable' GROUP BY PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') ORDER BY PPC.CLAIM_NO DESC "; //WHERE PPC.IS_CHECK = 'Incomplete'
            }
            else
            {
                 makeSQL = " SELECT PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME AS EMP_NAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, count(PPM.SLIP_NO) as SLIP_NO, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') AS PRINT_DATE FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID LEFT JOIN NRC_PAYMENT_TYPE NPT ON NPT.PAYMENT_TYPE_ID = PPC.PAYMENT_TYPE_ID LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID WHERE PPC.CLAIM_NO like '" + txtSearchUser.Text + "%' GROUP BY PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, PPC.IS_CHECK, PPC.IS_PRINT, TO_CHAR(PPC.PRINT_DATE, 'DD/MM/YYYY HH:MI:SS AM') ORDER BY PPC.CLAIM_NO DESC ";
                 alert_box.Visible = false;
            }
        ...
        public void TextClaimNo_TextChanged(object sender, EventArgs e)
        {
            ...
            if (ClaimNo != null)
            {
                ...
                if (Regex.IsMatch(ClaimNo, MatchEmpIDPattern))
                {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandText = "select CLAIM_NO from WP_PURCHASE_CLAIM where CLAIM_NO = '" + Convert.ToInt32(ClaimNo) + "'";
