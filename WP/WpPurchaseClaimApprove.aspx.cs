...
{
    public partial class WpPurchaseClaimApprove : System.Web.UI.Page
    {
        ...
        protected void BtnUpdate_Click(object sender, EventArgs e)
        {
            if (IS_EDIT_ACTIVE == "Enable")
            {
                ...
                {
                    string update_purchase_claim = " update  WP_PURCHASE_CLAIM set IS_OBJ_QUERY =:TextIsQuery, OBJ_QUERY_DES =:TextQueryDescription,  OBJ_QUERY_C_DATE = TO_DATE(:DateObjQuery, 'DD-MM-YYYY HH:MI:SS AM'), IS_CHECK =:TextIsCmoCheck , CHECK_USER_ID = :NoC_USER_ID where PURCHASE_CLAIM_ID =: NoPurClaimID ";
                ...
                else if (CheckIsCmo.Checked == true)
                {
                    ...
                    string update_purchase_claim = " update  WP_PURCHASE_CLAIM set IS_CHECK =:TextIsCmoCheck,   CHECK_DATE = TO_DATE(:DateCmoCheck, 'DD-MM-YYYY HH:MI:SS AM') , CHECK_USER_ID = :NoC_USER_ID where PURCHASE_CLAIM_ID =: NoPurClaimID ";
                    cmdu = new OracleCommand(update_purchase_claim, conn);
        ...
        protected void BtnUpdateQuery_Click(object sender, EventArgs e)
        {
            if (IS_EDIT_ACTIVE == "Enable")
            {
                ...
                string IsQuery = CheckIsQuery.Checked ? "Yes" : "No"; 
                    ...
                    string update_purchase_claim = " update  WP_PURCHASE_CLAIM set IS_OBJ_QUERY =:TextIsQuery, OBJ_QUERY_DES =:TextQueryDescription,  OBJ_QUERY_C_DATE = TO_DATE(:DateObjQuery, 'DD-MM-YYYY HH:MI:SS AM'),  CHECK_USER_ID = :NoC_USER_ID where PURCHASE_CLAIM_ID =: NoPurClaimID ";
                    cmdu = new OracleCommand(update_purchase_claim, conn);
        ...
        { 
             ...
             string makePageSQL = " SELECT PPM.SLIP_NO, PPM.SLIP_NO || ' - ' || PC.PARTY_ID || ' - ' || PC.PARTY_NAME || ', Item -' || PI.ITEM_NAME || ', Weight- ' || PPM.ITEM_WEIGHT || ', Amount- ' ||  TO_CHAR(PPM.TOTAL_AMOUNT, '999,999,999.99')  AS PARTY_NAME, nvl(PPM.TOTAL_AMOUNT,0) AS ITEM_AMOUNT  FROM WP_PURCHASE_MASTER PPM  LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID LEFT JOIN WP_ITEM PI ON PI.ITEM_ID = PPM.ITEM_ID  WHERE  PPM.CLAIM_NO = '" + USER_DATA_ID + "' ORDER BY  PPM.SLIP_NO ASC"; //  
             ...
             string makeSQL = " SELECT PURCHASE_CLAIM_ID, CLAIM_NO, EMP_ID, TO_CHAR(TO_DATE(CLAIM_DATE),'dd/mm/yyyy') AS CLAIM_DATE, TO_CHAR(TO_DATE(CLAIM_FOR_MONTH),'mm/yyyy') AS CLAIM_FOR_MONTH, PAYMENT_TYPE_ID, TOTAL_AMOUNT, IS_ACTIVE, OBJ_QUERY_DES, IS_CHECK, IS_OBJ_QUERY FROM WP_PURCHASE_CLAIM where CLAIM_NO = '" + USER_DATA_ID + "' ORDER BY CLAIM_NO DESC";
           ...
           //  decimal ItemAmountRound = Math.Round(Convert.ToDecimal(ItemAmount), 0, MidpointRounding.AwayFromZero);
             ...
             string makeSlipSQL = " SELECT PPC.CLAIM_NO, PPM.SLIP_NO FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO WHERE PPC.CLAIM_NO = '" + USER_DATA_ID + "' ORDER BY PPM.SLIP_NO ASC"; // PPC.IS_CHECK = 'Incomplete' AND  
        ...
        public void Display()
        {
            ...
            {
                makeSQL = " SELECT PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME AS EMP_NAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, count(PPM.SLIP_NO) as SLIP_NO, PPC.IS_CHECK FROM WP_PURCHASE_CLAIM PPC LEFT JOIN WP_PURCHASE_MASTER PPM ON PPM.CLAIM_NO = PPC.CLAIM_NO LEFT JOIN HR_EMPLOYEES HE ON HE.EMP_ID = PPC.EMP_ID LEFT JOIN NRC_PAYMENT_TYPE NPT ON NPT.PAYMENT_TYPE_ID = PPC.PAYMENT_TYPE_ID LEFT JOIN WP_PARTY PC ON PC.PARTY_ID = PPM.PARTY_ID WHERE PPC.IS_CHECK = 'Incomplete' AND PPC.IS_ACTIVE = 'Enable' GROUP BY PPC.PURCHASE_CLAIM_ID, PPC.CLAIM_NO, HE.EMP_FNAME || ' ' || HE.EMP_LNAME, PPC.CLAIM_FOR_MONTH, PPC.CLAIM_DATE, NPT.PAYMENT_TYPE_NAME, PPC.TOTAL_AMOUNT, PPC.CREATE_DATE, PPC.UPDATE_DATE, PPC.IS_ACTIVE, IS_OBJ_QUERY, PPC.OBJ_QUERY_DES, PPC.OBJ_QUERY_C_DATE, PPC.IS_CHECK ORDER BY PPC.CLAIM_NO DESC "; //WHERE PPC.IS_CHECK = 'Incomplete'
